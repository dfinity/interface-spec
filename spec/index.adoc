...
+
An external user can use these ids as the `sender` of a request if they own the corresponding private key.  The public key uses DER encoding as described in <<authentication>>.
 ...
 * Request fields that indicate public key (`sender_pubkey`, `public_key`) are binary blobs that contain DER encodings of the public keys.
