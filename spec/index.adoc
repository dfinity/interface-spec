...
A proposed change of this document should take the form of a Pull Request on GitHub, be titled “Public Spec: _title_”, and its description should answer the following questions:
 ...
 2. Which parties (Client, Languages, SDK, Research) are affected by this change, and why?
...
[TIP]
The canister with id `0xABCDE01` becomes `ic:ABCDE01A7` (https://crccalc.com/?crc=ABCDE01&method=crc8&datatype=hex&outtype=hex[online calculator]).
...
WARNING: This request type is currently accepted, but ignored.
...
The static assets (including, per convention, the canisters interface description) can be stored using a asynchronous request. Exiting assets of the given name are overwritten.

Synchronicity:: asynchronous
Request type:: `store_asset`
Request fields::
* `canister_id` (`CanisterId`): The id of the canister to query.
* `name` (`text`): Name of the asset to store
* `data` (`blob`): The content of the asset to store
Reply fields::
* None
...
WARNING: In the current implementation, anybody can call this request, as no list of admins or controllers is maintained. Eventually, only canister controllers should be allowed to store assets.

The static assets, as stored using the `store_asset` request can be fetched using a read request.

Synchronicity:: synchronous
Request type:: `retrieve_asset`
Request fields::
* `canister_id` (`CanisterId`): The id of the canister to query.
* `name` (`text`): Name (filename) of the asset to retrieve
Response fields::
* `status` (`text`): One of `replied` or `rejected`
* `reply`: If the status is `replied`, then this member contains the asset (as a blob).
* `reject_code` (`nat`): If the status is `rejected`, then this member contains the reject code (see <<reject-codes>>).
* `reject_message` (`text`): If the status is `rejected`, then this member contains a textual diagnostic message.
...
+
This traps if no argument data is available (e.g. in a reply callback).
...
Read response::
A record with
* `{status: received}` if `MS = Received`
...
+
....
{status: replied; reply: <R> }
....

==== Storing assets

Only an admin of the given canister can store assets. Existing assets are simply overriden.

Conditions::
....
    S.requests[StoreAsset SA] = Received
    C = S.canisters[SA.canister]
    C ≠ EmptyCanister
    M.sender ∈ S.admins[SA.canister]
....
State after::
....
S with
    canisters[SA.canister_id].assets[SA.name] = SA.data
....

==== Read: retrieving assets

A canister’s assets can be fetched.

Read request:: RetrieveAssetQuery Q
Conditions::
....
  C = S.canisters[Q.canister]
  C ≠ EmptyCanister
  F = C.assets[Q.name]
....
Read response::
