...
If the create-canister request includes a derivation nonce, the created id is derived from the caller's principal; otherwise a fresh opaque id is used.
...
A canister has an attribute _controller_; initially, the user who has registered the canister is the controller, but this can change using <<api-set-controller,the `set-controller` request>>.

Synchronicity:: asynchronous
Request type:: `create_canister`
Request fields::
* `sender` (`Principal`): The user who issued the request.
* `derivation_nonce` (`blob`): If present, indicates that the canister id should be derived from the sender principal.
Reply fields::
...
* `compute_allocation` (`nat`, optional): The initial compute allocation requested, in percent.
* `memory_allocation` (`nat`, optional): The initial memory allocation requested, in bytes.
* `mode` (string, one of `install`, `reinstall`, `upgrade`, optional default `install`): If `install`, the canister must be empty. If `upgrade`, indicates that this is an upgrade: the canister must already exist, be non-empty, and the existing version's <<system-api-upgrades,pre-upgrade hook>> must succeed. If `reinstall`, then the canister must be non-empty, but otherwise behaves like a fresh installation.
Reply fields::
* None

Only the _controller_ of the canister can install code.

If `mode = install` or `mode = reinstall`, this will instantiate the canister module and invoke its `canister_init` system method (if present), as explained in Section “<<system-api-init>>”, passing the `arg` to the canister. Upon reinstalling an existing canister, all state (including the stable memory) is cleared.
...
Initially, the user or canister that created a canister is its _controller_, but
control can be yielded to another user or canister.

Only the current controller of the canister can issue these requests.

Synchronicity:: asynchronous
Request type:: `set_controller`
Request fields::
+
* `sender` (`Principal`): The user who issued the request.
* `canister_id` (`CanisterId`): The id of the canister
* `controller` (`Principal`): The new controller of the canister.
Reply fields::
* None
...
S with
    ...
    controllers[CanisterId] = M.sender
....
...
....
    ...
    Old_module.pre_upgrade(Old_State, M.caller, S.time[M.canister_id]) = Return Stable_memory
...
Only a controller of the given canister can modify the set of controllers.

Conditions::
....
    S.requests[SetController M] = Received
    M.sender = S.controllers[M.canister_id]
....
State after::
....
S with
    requests[InstallCode M] = Completed { result = { } }
    controllers[M.canister_id] = M.controller
....
...
S with
    ...
    messages =
      S.messages ·
      ResponseMessage {
        origin = call_contexts[Ctxt_id].origin;
