...
where `signing_canister_id` is the id of the signing canister.

The `tree` must be a `well_formed` tree with

  lookup(/sig/<s>/<m>, tree) = Found ""

where `<s>` is the SHA-256 hash of the `seed` used in the public key and `m` is the SHA-256 hash of the payload to be signed.
--
...
==== Calls to non-running canisters are rejected

A call to a canister which is not `Running`, or does not have sufficient cycles to execute a call is automatically rejected.
...
....
    ...
    Visible_balance | Is_response = S.balances[M.receiver]
                    | otherwise   = S.balances[M.receiver] - MAX_CYCLES_PER_MESSAGE
...
if
  ...
  Cycles_used
     ...
     ≤ S.balances[M.receiver] + res.cycles_accepted
       + (if Is_response then MAX_CYCLES_PER_RESPONSE else 0)
...
then
  S with
    ...
    balances[M.receiver] =
      min(
        S.balances[M.receiver] + res.cycles_accepted + (if Is_response then MAX_CYCLES_PER_RESPONSE else 0)
        - Cycles_used
        - ∑ [ MAX_CYCLES_PER_MESSAGE + call.transferred_cycles | call ∈ res.new_calls ]
      , MAX_CANISTER_BALANCE)
else
  S with
...
The cycle accounting differs for call messages and for response messages:

* For call messages, the `MAX_CYCLES_PER_MESSAGE` cycles are deducted here.
* For response messages, cycles have already been set aside before. The unused portion of the `MAX_CYCLES_PER_RESPONSE` cycles are refunded afterwards.
...
The system chooses an appropriate canister id and instantiates a new empty canister identified by this id. The issuer of the request is set to be the controller of newly created canister. All cycles on this call are now the canister's initial cycles.
...
ic0.call_new<es>(
  ...
  if MAX_CYCLES_PER_RESPONSE > es.balance then Trap
