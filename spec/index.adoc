...
For periodic or time-based execution, the WebAssembly module can export a function with name `canister_global_timer`. This function is called if the canister has set its global timer (using the System API function `ic0.global_timer_set`) and the current time (as returned by the System API function `ic0.time`) has passed the value of the global timer. Once the function `canister_global_timer` is scheduled, the canister's global timer is reset. In particular, the function `canister_global_timer` won't be scheduled again unless the canister sets the global timer again (using the System API function `ic0.global_timer_set`). The global timer scheduling algorithm is implementation-defined.

`canister_global_timer` is triggered by the IC, and therefore has no arguments, no caller, and cannot reply or reject.

NOTE: While an implementation will likely try to keep the interval between the value of the global timer and the invocation of `canister_global_timer` within a few seconds, this is not formally part of this specification.
...
* `H`: from `canister_heartbeat` or `canister_global_timer`
...
The canister can set a global timer to make the system schedule a call to the exported `canister_global_timer` Wasm method after the specified time.
...
Passing zero as an argument to the function resets the timer and thus prevents the system from scheduling calls to the canister's `canister_global_timer` Wasm method.
...
SystemFunc = WasmState -> Trap { cycles_used : Nat; } | Return {
  new_state : WasmState;
  cycles_used : Nat;
}
...
CanisterModule = {
  ...
  heartbeat : (Env) -> SystemFunc
  global_timer : (Env) -> SystemFunc
...
+
If canister `C` exports a method with name `canister_global_timer`, the global timer of canister `C` is set, and the current time for canister `C` has passed the value of the global timer, the IC will create the corresponding call context and reset the global timer.
+
Conditions::
+
....
...
....
    ...
    ( M.entry_point = Heartbeat
      F = system_method_as_update(Mod.heartbeat, Env)
    )
    or
    ( M.entry_point = GlobalTimer
      F = system_method_as_update(Mod.global_timer, Env)
    )
...
The functions `query_as_update` and `system_method_as_update` turns a query function resp the heartbeat or global timer into an update function; this is merely a notational trick to simplify the rule:
....
...
system_method_as_update(f, env) = λ wasm_state →
...
S with
    canisters[CanisterId] = EmptyCanister
    time[CanisterId] = CurrentTime
    global_timer[CanisterId] = CurrentTime
    controllers[CanisterId] = [M.caller]
    freezing_threshold[CanisterId] = 2592000
...
init = λ (self_id, arg, caller, sysenv) →
  ...
  Return {new_state = es.wasm_state; new_global_timer = es.new_global_timer; cycles_used = es.cycles_used;}
....
+
...
callbacks = λ(callbacks, response, refunded_cycles, sysenv, available) → λ wasm_state →
  ...
  try
    ...
    Return {
      ...
      new_global_timer = es.global_timer;
    }
  with Trap
...
ic0.global_timer_set<es>(timestamp: i64) : i64 =
  if es.new_global_timer = NoGlobalTimer
  then
    es.new_global_timer := timestamp
    return es.params.sysenv.global_timer
  else
    let prev_global_timer = es.new_global_timer
    es.new_global_timer := timestamp
    return prev_global_timer
