...
The public entry points of canisters are called _methods_. Methods can be declared to be either _update methods_ (state mutation is preserved), _query methods_ (state mutation is discarded, no further calls can be made), or _composite query_ methods (state mutation is discarded, calls can be made to query and composite query methods of canisters on the same subnet).
