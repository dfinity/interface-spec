...
* `*` = `I G P U Q Ry Rt C` (NB: Not `(start)`)
...
CanisterModule = {
  ...
  pay_ingress : (CanisterId, MethodName, WasmState, Timestamp) -> Trap | Return AccessQuota
}
....
...
....
    ...
    C.module.pay_ingress (R.sender, R.method_name, C.wasm_state, S.time[R.canister_id]) = Return Quota
...
+
....
pay_ingress = λ (caller, method_name, wasm_state, time) →
  let es = ref {empty_execution_state with
      wasm_state = wasm_state;
      params = empty_params with { caller = caller; method_name = method_name; time = time };
    }
  try func<es>() with Trap then Trap
