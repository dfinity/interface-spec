...
Currently, only the `GET`, `HEAD`, and `POST` methods are supported for HTTP requests.
...
- There is no guarantee on which replica makes the first `POST` request. Since a single replica may be malicious, it might be that the first `POST` request to reach the remote server is maliciously modified. Therefore, it is recommended to verify the content of the request at the remote server and/or use additional integrity checks.
