...
....
    ...
    C = S.canisters[M.canister_id]
    C.module.pay_ingress (M.sender, M.method_name, C.wasm_state) = Return Quota
    Quota.limit ≥ Quota.cost + bucket_cost(S, M.canister_id, Quota.bucket)
....
State after::
....
S with
...
S with
    messages =
      ...
      FuncMessage {
        ...
        receiver = M.callee;
        entry_point = PublicMethod M.method_name M.caller M.data
        queue = M.queue;
        bucket = M.bucket;
        cost = M.cost;
      } ·
...
....
    ...
    ∀ CallMessage M ∈ S.messages. M.call_context ≠ Ctxt_id
    ∀ ctxt_ids.
