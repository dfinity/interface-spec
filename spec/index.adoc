...
Principals, like canister ids and user ids, are – as far as most uses of the system are concerned – binary blobs of length at most 29 bytes.
...
....
calculates self-authenticating ids. With `|cid|` we denote the length of the canister id, in bytes, encoded as a single byte.
...
CanStatus = Running | Stopping (List CallOrigin) | Stopped
....
...
The system chooses an appropriate canister id and instantiates a new (empty) canister identified by this id. The issuer of the request is set to be the controller of newly created canister, and the canister status is set to 'Running'.
...
1. First, any `stop_canister` call sets the state of the canister to `Stopping`; we record in the status all `stop_canister` requests which arrive at the canister while it is stopping (or stopped).
...
S with
    messages = Older_messages · Younger_messages
    S.status[A.canister_id] = Stopping [M.origin]
....
...
S with
    messages = Older_messages · Younger_messages
    S.status[A.canister_id] = Stopping (Origins · M.origin)
....
