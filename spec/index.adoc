...
The public key uses one of the encodings described in <<authentication>>.
...
* `sender_delegation` (`array` of maps, optional): a chain of <<authentication-delegation, delegations>>, starting with the one signed by `sender_pubkey` and ending with the one delegating to the key relating to `sender_sig`.
...
The plain authentication scheme applies if the `sender_pubkey` field contains a DER-encoded public key that is either:
...
The web authentication scheme applies if the `sender_pubkey` field contains a DER-wrapped public key in COSE format.
 ...
 * Request fields that indicate a public key (`sender_pubkey`, `key` in a delegation) are binary blobs that contain DER-wrapped COSE encodings of the public keys, as further described below.
 ...
 * `delegation` (`map`): Map with two fields:
   - `pubkey` (`blob`): DER-encoded public key as described in either <<authentication-plain>> or <<authentication-webauthn>>.
 ...
 * `signature` (`blob`): Signature as described in either <<authentication-plain>> or <<authentication-webauthn>>, depending on the key used for signing the delegation.

The `signature` field of a delegation contains a signature on the concatenation of the 27 bytes `\x1Aic-request-auth-delegation` (the domain separator) and the 32-byte <<api-hash-of-map, representation-independent hash>> of the map contained in the `delegation` field.
...
All signatures are created and verified as described in <<authentication-plain>> and <<authentication-webauthn>>, respectively, depending on the type of public key.
...
The function
....
mk_self_authenticating_id : Principal -> PublicKey
...
verify_envelope({ content = C, sender_pubkey = PK, sender_sig = Sig, sender_delegation = DS}, U, T)
  = U = mk_self_authenticating_id E.sender_pubkey
  ∧ PK' = verify_delegations(DS, PK, T)
  ∧ verify_signature ("\x0Aic-request" · hash_of_map(C), PK')

verify_delegations([], PK, T) = PK
verify_delegations([D] · DS, PK, T) = verify_delegations(DS, D.pubkey, T)
 ...
 * `is_system_assigned (mk_self_authenticating_id pk) = false` for possible principals p and public keys `pk` and
