...
The canister history consists of a list of canister changes and their total number. The system can drop the oldest canister changes from the list to keep its length bounded, but the total number of changes records all canister changes even if they have already been dropped from the list, i.e., the total number of changes never decreases.

Every canister change is described by the system timestamp at which it was performed, its origin (a user or a canister), and its type (canister creation, code change, or controllers change). The change origin includes the principal (called _originator_ in the following) that initiated the change and, if the originator is a canister, the index into the originator's canister history when the originator initiated the change (represented as the total number of the originator's changes at that time). Code changes are described by their type (code install, code reinstall, code upgrade, code uninstall) and, if applicable, the SHA-256 hash of the newly deployed canister module. Controllers changes are described by the full new set of the canister controllers after the change.
...
The list of canister changes can be trimmed, but the total number of recorded canister changes cannot be altered.

Conditions::
....
  S.canister_history[CanisterId] = {
      total_num_changes = N;
      change_entries = Older_changes · Newer_changes;
    }
....
State after::
....
