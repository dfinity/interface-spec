...
+
This system call traps if the size of stable memory exceeds 65536 pages.
...
+
This system call traps if the original number of pages exceeds 65536 (without growing the memory).
...
+
This system call traps if `src+size` exceeds the size of the WebAssembly memory or `offset+size` exceeds the size of the stable memory or exceeds 2^32.
...
+
This system call traps if `dst+size` exceeds the size of the WebAssembly memory or `offset+size` exceeds the size of the stable memory or exceeds 2^32.
