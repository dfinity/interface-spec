...
....
Read response::
* If `F(NoEntropy, Q.callee, Q.arg) = Trap` then a record with
+
....
{status: failed; error: "Query execution trapped"}
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Ok res` and `res.new_calls ≠ ()` then a record with
+
....
{status: failed; error: "Query execution tried to send further messages "}
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Ok res` and `res.response = NoResponse` then a record with
+
....
{status: failed; error: "Query execution did not issue a result" }
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Ok res` and `res.response = Reject errcode` then a record with
+
....
{status: failed; error: <toErrorMessage errcode> }
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Ok res` and `res.response = Reply R` then a record with
+
....
