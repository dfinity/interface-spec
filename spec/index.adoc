...
The canister history consists of a list of canister change entries and their total number. The system can drop the oldest canister change entries from the list to keep its length bounded (at least 1000 change entries are guaranteed to remain in the list), but the total number of changes records all canister changes even if the corresponding entries have already been dropped from the list, i.e., the total number of changes never decreases.
...
may_read_path(S, _, ["canister", cid, "history"]) = cid == ECID
...
state_tree(S) = {
  ...
  "canister":
    { canister_id :
        ...
        { "history" : CBOR(S.canister_history[canister_id]) }
