...
CanisterModule = {
  ...
  get_ingress_bucket : (MethodName, WasmState, Arg, Balance) -> Trap | Return (bucket : Blob, cost : i64)
  get_ingress_limit_per_second : (WasmState, Timestamp, Balance, bucket : Blob) -> Trap |  Return (NoLimit | limit : i64)
}
....
...
+
....
get_ingress_bucket = λ (method_name, wasm_state, arg, balance) →
...
+
....
get_ingress_limit_per_second = λ (wasm_state, time, balance, bucket) →
  Return NoLimit
....
