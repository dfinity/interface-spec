...
- whenever the certificate contains the path `/request_status/<request_id>/reply` whose value is Candid-encoded and the first argument is a record with a `canister_id` field of type `principal`, then the `canister_id` must be included in each delegation’s canister id range.
