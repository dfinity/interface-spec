...
function textual_decode() {
  ...
  base32 -d | xxd -p | cut -b9- | tr a-z A-Z
}
....
=====
...
+
 * Allowed encodings for the public key are DER for Ed25519 and ECDSA, or raw bytes for Ed25519; the verifier will detect the right encoding.
 ...
 * The signatures are encoded as raw bytes.
...
* `ic0.canister_balance : ( unit_src : i32, unit_size : i32 ) -> (i64)`
+
indicates the current balance of that unit. The number is the balance before the execution of the current message, minus a reserve for the execution of the current message (only if `unit` denotes cycles), minus any funds queued up to send via `ic0.call_new`, plus any funds accepted via `ic0.msg_funds_accept`.
+
