...
Unless `return_signatures` is set to `"no"` in the request, the response to a signed query call contains a non-empty list of signatures
...
Unless `return_certificate` is set to `"no"` in the request, the response to a signed query call contains a certificate containing
...
....
Q.return_signatures = null => Sigs = null
Q.return_signatures = "no" => Sigs = []
