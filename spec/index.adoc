...
The released versions of this document are targetted by the production implementation (e.g. `replica`, `dfx`). Releasing a new version of the spec does not, however, imply that the implementations will immediatelly target that version. Rather, deciding which version to target when is a deliberate, coordinated decision between the involved components.
...
Principals, i.e. ids like canister ids and user ids, are – as far as most uses of the system is concerned – binary blobs. There is, however, some structure to them to encode specific authentication and authorization behavior.
...
+
These have the form `H(|canister_id| · canister_id · public_key) · 0x02` (33 bytes).
+
...
WARNING: The `desired_id` cannot actually be used here. The reason is that `desired_id` would be computed from `sender`, but also `sender` is computed from `desired_id`.
...
The request can include a derivation nonce, then the created id is derived from the caller's principal; otherwise a fresh opaque id is used.
