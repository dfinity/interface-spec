...
+
The system records three callbacks, by noting the current function table entry at the index `_fun`. The `_env` value will be passed to these functions, so they need to have type `+(env : i32) -> ()+`. If they do not have this type, then `ic0.call_simple` traps.
+
...
+
If this traps (e.g. runs out of gas); the state changes from the `cleanup` function are discard, as usual, and no further actions are taken related to that call. Canisters likely want to avoid this from happening.
+
...
The function `query_to_update_func` turns a query function into an update function, this is merely a notational trick to simplify the message execution rules:
....
...
S with
    messages =
      ...
      FuncMessage {
        ...
        func = F (RM.response, S.time[S.call_contexts[RM.call_context].canister])
        queue = Unordered
      } ·
      Younger_messages
....
...
ic0.call_simple<es>(
  ...
  on_cleanup := es.wasm_state.store.table[cleanup_fun]
  ...
  let build_callback (params, func, env) =
    λ (self_id, wasm_state) →
      ...
      try func<es>(env)
      with Trap then cleanup(self_id,wasm_state)
      Return {
        new_state = es'.wasm_state;
