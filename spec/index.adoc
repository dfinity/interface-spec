...
The public entry points of canisters are called _methods_. Methods can be declared to be either _update methods_ (state mutation is preserved), _query methods_ (state mutation is discarded, no further calls can be made), or _composite query_ methods (state mutation is discarded, calls can be made to canisters on the same subnet).
...
External calls can be update calls, which can call both kinds of methods, query calls, which can _only_ call query methods, and composite query calls, which can _only_ call query and composite query methods.
...
4. _Anonymous id_
+
This has the form `0x04`, and is used for the anonymous caller. It can be used in call, query, and composite query requests without a signature.
...
* At `/api/v2/canister/<effective_canister_id>/composite_query` the user can perform (synchronous, non-state-changing) composite query calls.
...
Requests to `/api/v2/canister/<effective_canister_id>/call`, `/api/v2/canister/<effective_canister_id>/read_state`, `/api/v2/canister/<effective_canister_id>/query`, and `/api/v2/canister/<effective_canister_id>/composite_query` are POST requests with a CBOR-encoded request body, which consists of a authentication envelope (as per <<authentication>>) and request-specific content as described below.
...
This request type can _also_ be used to call a query method (but not a composite query method). A user may choose to go this way, instead of via the faster and cheaper <<http-query>> below, if they want to get a _certified_ response. Note that the canister state will not be changed by sending a call request type for a query method.
...
[#http-composite-query]
=== Request: Composite query call

A composite query call is a fast, but less secure way to call a canister. Only methods that are explicitly marked as “composite query methods” or “query methods” by the canister can be called this way. In contrast to a query call, a composite query call can make further calls to composite query methods of canisters on the same subnet.

In order to make a composite query call to canister, the user makes a POST request to `/api/v2/canister/<effective_canister_id>/composite_query`. The request body consists of an authentication envelope with a `content` map with the following fields:

* `request_type` (`text`): Always `composite_query`.
* `sender`, `nonce`, `ingress_expiry`: See <<authentication>>.
* `canister_id` (`blob`): The principal of the canister to call.
* `method_name` (`text`): Name of the canister method to call.
* `arg` (`blob`): Argument to pass to the canister method.
...
* If the request is an update call to a canister that is not the Management Canister (`aaaaa-aa`) or if the request is a query or composite query call, then the effective canister id must be the `canister_id` in the request.
...
As long as you talk to one (honest) node only, the observed behavior is nicely sequential. If you issue an update (i.e. state-mutating) call to a canister (e.g. bump a counter), and node A indicates that the call has been executed, and you then issue a query or composite query call to node A, then A's response is guaranteed to include the effect of the update call (and you will receive the updated counter value).
...
A related problem is that query and composite query calls are not certified, and nodes may be dishonest in their response. In that case, the user might want to get more assurance by querying multiple nodes and comparing the result. However, it is (currently) not possible to query a _specific_ state.
...
To define a public method of name `name`, a WebAssembly module exports a function with name `canister_update <name>`, `canister_query <name>`, or `canister_composite_query <name>` and type `+() -> ()+`. We call this the _method entry point_. The name of the exported function distinguishes update, query, and composite query methods.

NOTE: The space in `canister_update <name>`, `canister_query <name>`, and `canister_composite_query <name>`, resp., is intentional. There is exactly one space between `canister_update/canister_query/canister_composite_query` and the `<name>`.
...
* `*` = `I G U Q CQ Ry Rt C F T` (NB: Not `(start)`)
...
NOTE: The `canister_inspect_message` is _not_ invoked for query and composite query calls, inter-canister calls or calls to the management canister.
...
When executing a query method via a query call and a query or composite query method via a composite query call (i.e. in non-replicated state), the canister can fetch a certificate that authenticates to third parties the value last set via `ic0.certified_data_set`.

* `+ic0.data_certificate_present : () -> i32+`
+
returns `1` if a certificate is present, and `0` otherwise.
+
This will return `1` when called from a query method when invoked via a query call and when called from a query or composite query method via a composite query call.
+
This will return `0` for update methods and if a query method is executed within replicated execution (e.g. when invoked via an update call or inter-canister call).
...
2. Check whether the hostname is a _raw_ hostname (e.g., `<name>.raw.ic0.app`). If it is the case, fail and handle the request as a Web2 request, otherwise continue.
...
4. Check whether the canister is hosted on the IC using a custom domain. The client-side implementation can choose one of the following two options:
...
We distinguish between the _asynchronous_ API requests (type `Request`) passed to `/api/v2/…/call`, which may be present in the IC state, and the _synchronous_ API requests passed to `/api/v2/…/read_state`, `/api/v2/…/query`, and `/api/v2/…/composite_query`, which are only ephemeral.
 ...
 * Responses to reads (i.e. `/api/v2/…/read_state`, `/api/v2/…/query`, and `/api/v2/…/composite_query`). By definition, these do _not_ change the state of the IC, and merely describe the response based on the read request (or query and composite query, respectively) and the current state.
...
....
  ...
  Env = {
    time = S.time[Q.canister_id];
    global_timer = S.global_timer[Q.canister_id];
    balance = S.balances[Q.canister_id];
    freezing_limit = freezing_limit(S, Q.canister_id);
    certificate = Cert;
    status = simple_status(S.canister_status[Q.canister_id]);
    canister_version = S.canister_version[Q.canister_id];
  }
....
Read response::
* If `F(Q.Arg, Q.sender, Env)(S.canisters[Q.canister_id].wasm_state) = Trap trap` or `F(Q.Arg, Q.sender, Env)(S.canisters[Q.canister_id].wasm_state) = Return {response = NoResponse; …}` then
+
....
{status: rejected; reject_code: CANISTER_ERROR, reject_message: <implementation-specific>, error_code: <implementation-specific>}
....
* Else if `F(Q.Arg, Q.sender, Env)(S.canisters[Q.canister_id].wasm_state) = Return {response = Reject (code, msg); …}` then `code = CANISTER_REJECT` and
+
....
{status: rejected; reject_code: CANISTER_REJECT: reject_message: <msg>, error_code: <implementation-specific>}
....
* Else if `F(Q.Arg, Q.sender, Env)(S.canisters[Q.canister_id].wasm_state) = Return {response = Reply R; …}` then
+
....
{status: success; reply: { arg : R } }
....

==== Composite query call

Canister composite query calls to `/api/v2/canister/<ECID>/composite_query` can be executed directly. They can only be executed against canisters which have a status of `Running` and are also not frozen.

During the execution of a composite query call, a certificate is provided to the canister that is valid, contains a current state tree (or “recent enough”; the specification is currently vague about how old the certificate may be), and reveals the canister’s <<system-api-certified-data,Certified Data>>.

....
composite_query_helper(S, Cycles, Caller, Canister_id, Method_name, Arg) =
  let Mod = S.canisters[Canister_id].module
  let F = Mod.composite_query_methods[Method_name]
  ...
  if S.canisters[Canister_id] ≠ EmptyCanister and
     ...
     Method_name ∈ dom(S.canisters[Canister_id].module.composite_query_methods) and
  ...
  then
     ...
     then
        ...
        then
           ...
           do
              let (Call, _) = Calls
              let (Response', Cycles_used') = composite_query_helper(S, Cycles, Canister_id, Call.callee, Call.method_name, Call.arg)
              Cycles := Cycles - Cycles_used'
              if Cycles < MAX_CYCLES_PER_RESPONSE or
                 S.canister_subnet[Canister_id] ≠ S.canister_subnet[Call.callee]
              then
                 Return (Reject (CANISTER_ERROR, <implementation-specific>), Cycles) // composite query out of cycles or calling to another subnet
              let F' = Mod.callbacks(Call.callback, Response', 0, Env, 0)
              let R'' = F'(W')
              if R'' = Trap trap''
              then Return (Reject (CANISTER_ERROR, <implementation-specific>), Cycles - trap''.cycles_used)
              else if R'' = Return {new_state = W''; new_calls = Calls''; response = Response''; cycles_used = Cycles_used''}
              then
                 ...
                 if Response'' = NoResponse
                 then
                    Calls := Calls'' · Calls // DFS call graph traversal
                 else
...
Conditions::
....
  E.content = CanisterCompositeQuery Q
  Q.canister_id ∈ verify_envelope(E, Q.sender, S.system_time)
  is_effective_canister_id(E.content, ECID)
  S.system_time <= Q.ingress_expiry
....
Read response::
* if `composite_query_helper(S, MAX_CYCLES_PER_COMPOSITE_QUERY, Q.sender, Q.canister_id, Q.method_name, Q.arg) = Return {response = Reject (code, msg); …}` then `code = CANISTER_REJECT` and
+
....
{status: rejected; reject_code: CANISTER_REJECT: reject_message: <msg>, error_code: <implementation-specific>}
....
* Else if `composite_query_helper(S, MAX_CYCLES_PER_COMPOSITE_QUERY, Q.sender, Q.canister_id, Q.method_name, Q.arg) = Return {response = Reply R; …}` then
+
....
{status: success; reply: { arg : R } }
....
...
ic0.msg_arg_data_size<es>() : i32 =
  if es.context ∉ {I, U, Q, CQ, Ry, F} then Trap {cycles_used = es.cycles_used;}
...
ic0.msg_reply_data_append<es>(src : i32, size : i32) =
  if es.context ∉ {U, Q, CQ, Ry, Rt} then Trap {cycles_used = es.cycles_used;}
...
ic0.call_new<es>(
  ...
  if es.context ∉ {U, CQ, Ry, Rt, T} then Trap {cycles_used = es.cycles_used;}
