...
Note that `WasmFunc` is an abstract notion of a WebAssembly-state-modifying-function, not merely a concrete function in a WebAssembly module. In particular,
it abstracts the bookkeeping of `fun` and `env` arguments to `call.onreply` and `call.onreject` that we encountered in Section <<system-api>>. `WasmFunc`s model a form of closure, which (currently) do not exist on the WebAssembly level.

The `Entropy` parameter of a `WasmFunc` is merely passed through to the canister, via the `msg.get_entropy` system call.
...
....
    ...
    F = S.canisters[CM.callee].module.exports[CM.method_name]
...
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Return res` and `res.new_calls ≠ ()` then a record with
+
....
{status: failed; error: "Query execution tried to send further messages "}
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Return res` and `res.response = NoResponse` then a record with
+
....
{status: failed; error: "Query execution did not issue a result" }
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Return res` and `res.response = Reject errcode` then a record with
+
....
{status: failed; error: <toErrorMessage errcode> }
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Return res` and `res.response = Reply R` then a record with
+
....
