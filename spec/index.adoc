...
External calls can be update calls, which can call update and query methods, query calls, which can _only_ call query methods, and composite query calls, which can _only_ call query and composite query methods.
Inter-canister calls can call update and query methods (just like update calls). Note that calls from a canister to itself also count as "inter-canister".
...
We define an auxiliary method that handles calls from composite query methods by performing a call graph traversal.
....
composite_query_helper(S, Cycles, Caller, Canister_id, Method_name, Arg) =
  ...
  if S.canisters[Canister_id] ≠ EmptyCanister and
     ...
     Method_name ∈ dom(Mod.composite_query_methods) and
...
....
Read response::
* if `composite_query_helper(S, MAX_CYCLES_PER_COMPOSITE_QUERY, Q.sender, Q.canister_id, Q.method_name, Q.arg) = (Reject (code, msg), _)` (note that `code = CANISTER_REJECT` in this case) then
+
....
{status: rejected; reject_code: CANISTER_REJECT: reject_message: <msg>, error_code: <implementation-specific>}
....
* Else if `composite_query_helper(S, MAX_CYCLES_PER_COMPOSITE_QUERY, Q.sender, Q.canister_id, Q.method_name, Q.arg) = (Reply R, _)` then
+
....
{status: success; reply: { arg : R } }
....
