...
* The canister may export a function named `canister_init` with type `+() -> ()+`.
* The canister may export a function named `canister_pre_upgrade` with type `+() -> ()+`.
* The canister may export a function named `canister_post_upgrade` with type `+() -> ()+`.
* The canister may export a function named `canister_pay_ingress` with type `+() -> ()+`.
* The canister may export functions named `canister_update <name>` with type `+() -> ()+`.
* The canister may export functions named `canister_query <name>` with type `+() -> ()+`.
 ...
 * `is_system_assigned (mk_self_authenticating_id cid pk) = false` for all possible canister ids `cid` and public keys `pk` and
 * `is_system_assigned (mk_derived_id p nonce) = false` for all `p` that could be a user or canister.
