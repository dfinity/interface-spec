...
You can interact with the DFINITY Computer using an RPC interface, called the _Cloud API_, which exposes a small number of endpoints that take structured requests.
...
NOTE: I expect that the message id will be calculated from the message data, so there is no explicit `message_id` returned from the node, but this is contested, and returning a `message_id` will not fundamentally affect this design.
...
The symbolic names of this enumeration is used throughout this specification, but on all interfaces (Cloud API, System API), they are represented as positive numbers, i.e.`SYS_FATAL=1`, `SYS_TRANSIENT=1` etc.
...
NOTE: These messages are a different thing from the messages that occur in the Cloud API; the latter are called API Messages in this document.
...
If there is no call, downstream calling context or response that could possibly fulfill a calling context, then a reject is synthesized. The error message below is _not_ indicative. In particular, if the system has an idea about _why_ this starved, it can put that in there (e.g. the inital message handler trapped with an out-of-memory access).
