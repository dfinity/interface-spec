...
This document describes this _external_ view of the Internet Computer, i.e. the low-level interfaces it provides to application developers and users, and what will happen when they use these interfaces.
...
If you want to use the Internet Computer as an application developer, you first create a _canister module_ that contains the WebAssembly code and configuration for your application, and deploy it using the <<http-interface,HTTPS interface>>. You can create canisters using the Motoko language and the SDK, which is more convenient. If you want to use your own tooling, however, then this document describes <<canister-module-format, what a canister module looks like>> and how the <<system-api,WebAssembly code can interact with the system>>.

Once your application is running on the Internet Computer, it is a _canister_, and users can interact with it. They can use the <<http-interface,HTTPS interface>> to interact with the canister according to the <<system-api,System API>>.
...
Principals are generic identifiers for canisters, users and possibly other concepts in the future. As far as most uses of the system are concerned they are _opaque_ binary blobs with a length between 0 and 29 bytes, and there is intentionally no mechanism to tell canister id and users ids apart.
...
* At `/api/v2/canister/<effective_canister_id>/read_state` the user can read various system information. In particular, they can poll for the status of a call here.
...
3. The system asks the targeted canister if it is willing to accept this message and be charged for the expense of processing it. This uses the <<system-api-inspect-message>> API for normal calls. For calls to the management canister, the rules in <<ic-management-canister>> apply.
...
The characteristic property of the `received` state is that the call has made it past the (potentially malicious) endpoint _into the system_. It is now pointless (but harmless) to submit the (identical) call again. Before reaching that state, submitting the identical call to further nodes might be a useful safeguard against a malicious or misbehaving node.
 ...
 * `/canisters/<canister_id>/controllers`. Can be requested by anyone, if `<canister_id>` matches `<effective_canister_id>`. The order is system-defined.
...
NOTE: The expectation is that user-side agent code shields users and developers from this concept, in analogy to how the system interface shields canister developers from worrying about routing.
...
* The canister may export a function named `canister_heartbeat` with type `+() -> ()+`.
...
For periodic or time-based execution, the WebAssembly module can export a function with name `canister_heartbeat`. If present, the system will invoke this function at regular intervals. The exact interval is implementation-defined. For each heartbeat invocation, the system guarantees that the time, as returned by <<system-api-time,`ic0.time`>>, is monotonically increasing.

`canister_heartbeat` is triggered by the system, and therefore has no arguments, no caller, and cannot reply or reject.
...
The system records two mandatory callback functions, represented by a table entry index `*_fun` and some additional value `*_env`. When the response comes back, the table is read at the corresponding index, expected to be a function of type `+(env : i32) -> ()+`, and passed the corresponding `*_env` value.
...
+
(deprecated) This adds cycles onto a call. See <<system-api-cycles>>.
+
This may be called multiple times between `ic0.call_new` and `ic0.call_perform`.
...
+
If the system returns `0` as the `err_code`, the system was able to enqueue the call. In this case, the call will either be delivered, returned because the destination canister does not exist or returned because of an out of cycles condition. This also means that exactly one of the reply or reject callbacks will be executed.
+
...
+
NOTE: This call is deprecated and traps if the current balance does not fit into a 64-bit value. Canisters that need to deal with larger cycles balances should use `ic0.canister_cycles_balance128` instead.

* `ic0.canister_cycle_balance128 : () -> (high : i64, low : i64)`
+
indicates the current cycle balance of the canister. It is the canister balance before the execution of the current message, minus a reserve to pay for the execution of the current message, minus any cycles queued up to be sent via `ic0.call_cycles_add128`. After execution of the message, the system may add unused cycles from the reserve back to the balance.
+
NOTE: Cycles are represented by 128-bit values. The amount of cycles that the canister's balance holds can be obtained by combining the return values: high * 2^64^ + low.
...
+
NOTE: This call is deprecated and traps if the amount of cycles available does not fit into a 64-bit value. Please use `ic0.msg_cycles_available128` instead.

* `ic0.msg_cycles_available128 : () -> ( high: i64, low : i64)`
+
returns the amount of cycles that were transferred by the caller of the current call, and is still available in this message.
+
...
+
NOTE: Cycles are represented by 128-bit values. The amount of cycles transferred can be obtained by combining the return values: high * 2^64^ + low.
...
This system call is deprecated and does not trap.
...
* `ic0.msg_cycles_accept128 : ( max_amount_high : i64, max_amount_low : i64 ) -> ( amount_high : i64, amount_low : i64 )`
+
--
This moves cycles from the call to the canister balance. It moves as many cycles as possible, up to these constraints:
...
The amount of cycles that were actually moved can be obtained by combining the return values: amount_high * 2^64^ + amount_low.
...
+
This system call is deprecated and traps if trying to transfer more cycles than are in the current balance of the canister.

* `ic0.call_cycles_add128 : ( amount_high : i64, amount_low : i64 ) -> ()`
+
This function moves cycles from the canister balance onto the call under construction, to be transferred with that call.
+
The amount of cycles it moves is represented by a 128-bit value which can be obtained by combining `amount_high` and `amount_low` parameters.
+
...
NOTE: This call is deprecated and traps if the amount of cycles refunded does not fit into a 64-bit value. Please use `ic0.msg_cycles_refunded128` instead.

* `ic0.msg_cycles_refunded128 : () -> ( high : i64, low : i64 )`
+
this can only be used in a callback handler (reply or reject), and indicates the amount of cycles that came back with the response as a refund. The refund has already been added to the canister balance automatically.

NOTE: Cycles are represented by 128-bit values. The amount of cycles refunded can be obtained by combining the return values: high * 2^64^ + low.
...
+
This system call is deprecated and traps if the size of stable memory exceeds 65536 pages.
...
+
This system call is deprecated and traps if the original number of pages exceeds 65536 (without growing the memory).
...
+
This system call is deprecated and traps if `src+size` exceeds the size of the WebAssembly memory or `offset+size` exceeds the size of the stable memory or exceeds 2^32.
...
+
This system call is deprecated and traps if `dst+size` exceeds the size of the WebAssembly memory or `offset+size` exceeds the size of the stable memory or exceeds 2^32.
...
The canister can query the "performance counter", which is a deterministic monotonically increasing integer approximating the amount of work the canister performed since the beginning of the message execution.

The counter is reset each time a new message is passed to the canister for execution.

The exact semantics of the counter is implementation-defined and can depend on:

  * The number of WebAssembly instructions executed so far.
  * The number of canister linear memory pages dirtied.
  * The number of bytes copied to and from the system state.
...
1. The introduction of a `api_nonce` reference, which models the capability to use the System API. It is passed as an argument to `canister_init`, `canister_update <name>` etc., and expected as an argument by almost all system function calls. (The debugging aids remain unconstrained.)
...
+
A list of principles. Must be between 0 and 10 in size. This value is assigned to the _controllers_ attribute of the canister.
+
...
Only _controllers of the canister can install code.
...
Only _controllers of the canister can uninstall code.
...
Only _controllers of the canister can delete it and the canister must already be stopped. Deleting a canister cannot be undone, any state stored on the canister is permanently deleted and its cycles are discarded. Once a canister is deleted, its ID cannot be reused.
...
All state trees include the time at path `/time` (see <<state-tree-time>>). Clients that get a certificate with a state tree can look up the timestamp to guard against working on obsolete data.

=== Lookup

Given a (verified) tree, the client can fetch the value at a given path, which is a sequence of labels (blobs). In this document, we write paths suggestively with slashes as separators; the actual encoding is not actually using slashes as delimiters.
...
The system will only produce well-formed state trees, and the above algorithm assumes well-formed trees. These have the property that labeled subtrees appear in strictly increasing order of labels, and are not mixed with leaves. More formally:
....
well_formed(tree) =
...
Delegation are _scoped_, i.e. they indicate which set of canister ids the delegatee subnet may certify for. This set can be obtained from a a delegation `d` using `lookup(["subnet",d.subnet_id,"canister_ranges"],d.certificate)`, which must be present, and is encoded as described in <<state-tree-subnet>>. The various applications of certificates describe if and how the subnet scope comes into play.
...
+
If canister `C` exports a method with name `canister_heartbeat`, the system will add a message to the head of the queue for invoking `canister_heartbeat` and create the corresponding call context.
+
Conditions::
+
....
...
The system chooses an appropriate canister id and instantiates a new (empty) canister identified by this id. The _controllers_ are set such that the sender of this request is the only controller, unless the `settings` say otherwise. All cycles on this call are now the canister's initial cycles.
...
Only the controllers of the given canister can install code. This transition installs new code over a canister. This involves invoking the `canister_init` system method (see <<system-api-init>>), which must succeed.
...
Only the controllers of the given canister can install new code. This changes the code of an _existing_ canister, preserving the state in the stable memory. This involves invoking the `canister_pre_upgrade` system method on the old and `canister_post_upgrade` system method on the new canister, which must succeed and must not invoke other methods.
...
ic0.performance_counter<es>() : i64 =
  arbitrary()
