...
Request to `/api/v2/<effective_canister_id>/call`, `/api/v2/<effective_canister_id>/read_state` and `/api/v2/<effective_canister_id>/query` are POST requests with a CBOR-encoded request body, which consists of a authentication envelope (as per <<authentication>>) and request-specific content as described below.
...
3. At some point, the system may accept the call for processing and set its status to `received`. This indicates that the system as a whole has received the call and plans on processing it (although it may still not get processed if the system is under high load). Furthermore, the user should also be able to ask any endpoint about the status of the pending call.
...
5. Now the system is processing the call. For some calls this may be atomic, for others this involves multiple internal steps.
...
7. In the case that the call has been retained for long enough, but the request has not expired yet, the system can forget the response data and only remember the call as `done`, to prevent a replay attack.
8. Once the expiry time is past, the system can prune the call and its response, and completely forget about it.
...
If the canister does not implement `canister_inspect_message`, then no ingress message inspection is applied and the canister will receive the update calls.
...
....
    ...
    Arg = { data = E.content.data; caller = E.content.sender; method = E.content.method_name; time = S.time[E.content.canister_id] }
    C.module.inspect_message (E.content.method_name, C.wasm_state, Arg, S.balance[E.content.canister_id]) = Return Accept
....
State after::
....
S with
