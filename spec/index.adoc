...
....
Read response::
* if `composite_query_helper(S, MAX_CYCLES_PER_COMPOSITE_QUERY, Q.sender, Q.canister_id, Q.method_name, Q.arg) = Return {response = Reject (code, msg); …}` (note that `code = CANISTER_REJECT` in this case) then
+
....
{status: rejected; reject_code: CANISTER_REJECT: reject_message: <msg>, error_code: <implementation-specific>}
....
