...
In all of this document, `H` denotes SHA-256 and `·` denotes blob concatenation. There are several classes of ids:
...
+
These have the form `H(public_key · canister_id) · 0x02` (33 bytes).
+
...
3. _Derived ids_
+
These have the form `H(registering_principal) · (arbitrary 8 bytes) · 0x03` (41 bytes).
+
...
* `desired_id` (`Principal`, optional): The id the system should use for the canister
Reply fields::
...
The function
....
is_self_authenticating_id : PublicKey -> CanisterId -> PrincipalId -> Bool
is_self_authenticating_id pk canister_id id = id == H(pk · canister_id) · 0x02
....
characterizes the self-authenticating ids.

The function
....
is_derived_id : Principal -> Principal -> Bool
is_derived_id oid id = ∃n. |n| == 8 ∧ id == H(oid) · n · 0x03
....
characterizes the derived ids.

The function
....
is_anonymous_id : Principal -> Bool
is_anonymous_id id = id == 0x04
....
...
verify_envelope(E, U) =
  ...
  || ∃ PK, SIG.
       is_self_authenticating_id E.sender_pubkey E.content.canister_id U
