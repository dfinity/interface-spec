 ...
 * `/canisters/<canister_id>/metadata/<name>`. Can be requested if `<canister_id>` matches `<effective_canister_id>` and
...
If a path cannot be requested, then it is absent in the returned certificate or the HTTP response to the read state request has a 4xx status code.
