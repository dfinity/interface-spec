 ...
 * `/request_status/<request_id>`. Can be requested if no path with such a prefix exists in the state tree or
   ...
   - `<effective_canister_id>` is a valid effective canister id for the original request referenced by `<request_id>` and
   - the effective canister id of the original request referenced by `<request_id>` and the effective canister id `<effective_canister_id>` of the read state request belong to canister ranges of the same subnet.
