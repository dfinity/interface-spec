...
Aspects of this document not implemented in the master version of the official client are labeled as such, using the “warning” admonition.
...
For all endpoints, the client performs a POST request over HTTPS with `Content-type: application/cbor`. The body is an CBOR value containing the request object.
