...
2. For a certain amount of time, the system behaves as if it does not know about the request. (Althought as part of the HTTP interface the receiving endpoint gives an untrusted acknowledgment of receipt or an untrusted declination of the request.)
3. At some point, the system may accept the request for processing (or it expires). From now on, the user can ask any endpoint (for the canister) about the status of the pending request. Initially, the status is `received`: The system as a whole (not just a single node) has received the request, but it may still decide not to perform it, e.g. because of high load.
...
if "" as X then
  ...
  "submit" --> "expired" #DDDDDD
endif
....

Note that all gray states are _not_ represented in the system state, and are indistinguishable from “request does not exist”. In order to avoid replay-attacks, messages (will) have an expiry date, and the last transition (forgetting the message) must happen no earlier than the message’s expiry field. Further more requests must stay in `Replied` or `Rejected` long enough (precise number TBD) for clients to catch the response when polling.
