...
+
These have one of the forms `H(public_key) · 0x02` or `H(public_key) · 0x06` (both 29 bytes).
+
An external user can use these ids as the `sender` of a request if they own the corresponding private key.
The public key uses either DER encoding (for trailing `0x02`) or COSE encoding (for trailing `0x06`) as described in <<authentication>>.
...
If the principal of the sender ends with `0x02`, then the signature schemes used to authenticate users are
 ...
 * Request fields that indicate public key (`sender_pubkey`) are binary blobs that contain public keys.
   Allowed encodings are DER for Ed25519 and ECDSA, or raw bytes for Ed25519; the system will detect the right encoding.
...
The `sender_sig` is calculated by signing the concatenation of the 11 bytes `"\x0Aic-request"` (the domain separator) and the 32 byte <<api-request-id, _request id_>> with the secret key that belongs to the public key specified in `sender_pubkey`.

NOTE: Raw bytes for Ed25519 will be removed in a future version of the spec.

[#authentication-webauthn]
==== Web authentication (WebAuthn)

If the principal of the sender ends with `0x06`, then the signature scheme used to authenticate users is

 * https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.186-4.pdf[*ECDSA*] on curve P-256 (also known as `secp256r1`), using SHA-256 as hash function.

In particular:

 * Request fields that indicate public key (`sender_pubkey`) are binary blobs that contain COSE encodings of the public keys.
   See https://www.w3.org/TR/webauthn/#sctn-encoded-credPubKey-examples[WebAuthn w3c recommendation] or https://tools.ietf.org/html/rfc8152#section-13.1[RFC 8152] for details.
 * Request fields that indicate signatures (`sender_sig`) are binary blobs that contain a CBOR encoding of a map with three mandatory fields:
...
The 32 byte <<api-request-id, _request id_>> is used as the challenge in the assertion.
The signature is checked by verifying that the request id appears in base64url encoding (see https://tools.ietf.org/html/rfc4648#section-5[RFC 4648]) in the `challenge` field of `clientDataJSON`, and that `signature` verifies on `authenticatorData || SHA-256(clientDataJSON)`, as specified in the https://www.w3.org/TR/webauthn/#op-get-assertion[WebAuthn w3c recommendation].

Signing transactions can be delegated from a Web Authentication key to a key used in plain authentication. If delegation is used, The `sender_pubkey` field is still set to the COSE encoding of the public key used in WebAuthn. The `sender_sig` field is a binary blob containing a CBOR encoding with the same fields as above, and additionally:

 * `delegation` (`blob`): CBOR-encoding of a map with two mandatory fields:
   - `key` (`blob`): DER-encoded public key as described in <<authentication-plain>>, i.e. Ed25519 or ECDSA.
   - `expiration` (`nat`): Expiration of the delegation, in nanoseconds since 1970-01-01, analogously to the `ingress_expiry` field in <<authentication>>.
 * `delegate_signature` (`blob`): Signature as described in <<authentication-plain>>.

The fields `delegation` and `delegate_signature` must be both absent or both present.
If both fields are present, then the 32 byte <<api-request-id, _request id_>> is signed using the plain signature scheme relative to the key specified in the `delegation` object.
The challenge in the WebAuthn assertion is set to `SHA-256(delegation)`, where `delegation` refers to the encoded CBOR object.
...
Indicates the current status of the canister. It could be one of `Running`, `Stopping` or `Stopped`.
...
The function
....
is_self_authenticating_id : Principal -> PublicKey -> Bool
is_self_authenticating_id p pk = (p = H(pk) · 0x02) || (p = H(pk) · 0x06)
....
verifies whether a principal is self-authenticating for a specific public key.
...
CanisterModule = {
  ...
  get_ingress_bucket : (UserId, MethodName, WasmState Arg, Timestamp) -> Trap | Return (bucket : Blob, cost : i64)
  get_ingress_limit_per_second : (UserId, WasmState, Timestamp, bucket : Blob) -> Trap |  Return (NoLimit | limit : i64)
}
....
...
In particular, the function implements  https://ed25519.cr.yp.to/index.html[*Ed25519*] and https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.186-4.pdf[*ECDSA*] signatures, depending on the type of public key, for the plain format, as well as ECDSA signatures for the WebAuthn format.
....
PublicKey = Blob
Signature = Blob
...
....
    ...
    C.module.get_ingress_bucket (R.sender, R.method_name, C.wasm_state, R.data, S.time[R.canister_id]) = Return (assigned_bucket, assigned_cost)
    C.module.get_ingress_limit_per_second : (R.sender, C.wasm_state, S.time[R.canister_id], assigned_bucket) = Return assigned_limit
 ...
 * `is_system_assigned p && is_self_authenticating_id p pk = false` for possible principals p and public keys `pk` and
...
+
....
get_ingress_bucket = λ (caller, method_name, wasm_state, data, time) →
...
+
....
get_ingress_limit_per_second = λ (caller, wasm_state, time, bucket) →
  Return NoLimit
....
...
get_ingress_limit_per_second = λ (caller, wasm_state, time, bucket) →
   let es = ref {empty_execution_state with
       wasm_state = wasm_state;
       params = empty_params with { time = time, ingress_rate_bucket = bucket };
     }
   try func<es>() with Trap then Trap
   if es.calls ≠ () then Trap
   if es.response ≠ NoResponse then Trap
...
ic0.ingress_rate_bucket_size<es>() =
  if es.in_cleanup then Trap
  if es.params.ingress_rate_bucket = NoBucket then Trap
...
ic0.return_ingress_bucket<es>(bucket_src : i32, bucket_size : i32, cost : i64) =
  ...
  if |es.bucket_cost.bucket| > 64 then Trap
