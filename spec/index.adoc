 ...
 * `/request_status/<request_id>`. Can be requested if the effective canister id of the original request referenced by `<request_id>` matches `<effective_canister_id>` and
   - no path with such a prefix exists in the state tree or
   - the sender of the original request referenced by `<request_id>` is the same as the sender of the read state request.
...
If a path cannot be requested, then it is absent in the returned certificate (paths with prefix `/request_status/<request_id>` where the effective canister id of the original request referenced by `<request_id>` and the effective canister id `<effective_canister_id>` of the read state request belong to canister ranges of two different subnets) or the HTTP response to the read state request has a 4xx status code.

Read state requests containing many requested paths might be rejected with a 4xx HTTP status code.

[NOTE]
====
The Internet Computer blockchain mainnet rejects all read state requests requesting more than 100 paths with prefix `/request_status/<request_id>`.
====
