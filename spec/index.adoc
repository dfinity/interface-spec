...
In this section, `H` denotes SHA-256 and `·` denotes blob concatenation.
...
+
These ids are treated specially when an id needs to be registered. In such a request, the caller (a user or a canister) may indicate a desired id of this form, if `registering_principal` is his own id. See for example <<ic-create_canister>>.
...
The function
....
is_derived_id : PrincipalId -> PrincipalId -> Bool
...
verify_envelope(E, U) =
  ∃ PK, SIG.
    is_self_authenticating_id E.sender_pubkey U
...
....
    ...
    ( M.entry_point = Callback Callback Response
      F = Mod.callbacks(Callback, Response)
    )
...
If the call indicates a desired id, the system checks that it is derived from the caller id and not yet used; otherwise it creates a fresh canister id.
...
init = λ (self_id, arg) →
  let es = ref {empty_execution_state with
      ...
      params = empty_params with { data = arg.data; caller = arg.caller }
    }
...
pre_upgrade = λ (old_state, caller) →
  let es = ref {empty_execution_state with
      ...
      params = { empty_params with caller = caller }
    }
  try func<es>() with Trap then Trap
  if es.performed_calls ≠ [] then Trap
  if es.response ≠ NoResponse then Trap
