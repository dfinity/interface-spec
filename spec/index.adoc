...
NOTE: Once the IC frees the resources of a canister, its id, _cycles_ balance, _controllers_, canister _version_, and total number of canister changes are preserved on the IC for a minimum of 10 years. What happens to the canister after this period is currently unspecified.
...
The system can drop the oldest canister changes from the list to keep its length bounded (at least `1000` changes are guaranteed to remain in the list). The system also drops all canister changes if the canister runs out of cycles.
...
* `num_requested_changes`: optional, specifies the number of requested canister changes. The call will be charged based on this parameter. If not provided, the maximum of `1000` will be used.
...
* `changes`: the list containing the most recent canister changes. If `num_requested_changes` is provided, then this list contains that number of changes or all changes available in the history if more changes are requested than available in the history.
...
S with
    ...
    canister_history[CanisterId] = {
      ...
      changes = {
        ...
        canister_version = 0
        change_origin = canister_change_origin(M.caller, A.sender_canister_version, M.origin)
        change_details = CanisterCreation
      }
    }
    messages = Older_messages · Younger_messages ·
      ResponseMessage {
    ...
    if A.settings.controllers is not null:
      ...
      canister_history[A.canister_id] = {
        ...
        changes = H · {
            ...
            change_origin = canister_change_origin(M.caller, A.sender_canister_version, M.origin);
            change_details = ControllersChange {
...
Once a canister runs out of cycles, its code is uninstalled (cf. <<rule-uninstall>>), the canister history changes dropped (their total number is preserved), and the allocations are set to zero (NB: allocations are currently not modeled in the formal model):
...
The list of canister changes can be trimmed, but the total number of recorded canister changes cannot be altered. At least 1000 changes are guaranteed to remain in the list of changes.

Conditions::
....
  S.canister_history[CanisterId] = {
      total_num_changes = N;
      changes = Older_changes · Newer_changes;
    }
  |Newer_changes| ≥ 1000
....
State after::
