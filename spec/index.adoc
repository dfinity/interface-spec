...
The document is currently constantly moving, at a pointer somewhere in the triangle between what is implemented, what is being designed and what is being approved, in a best-effort way.
...
* `+ic0.msg_arg_data_copy : (dst : i32, offset : i32, length : i32) -> ()+`
+
...
+
  ...
  ic0.msg_caller_copy : (dst : i32, offset: i32, length : i32) -> ()
+
...
* `+ic0.msg_reply_data_append : (src : i32, length : i32) -> ()+`
+
...
* `+ic0.msg_reject : (src : i32, length : i32) -> ()+`
+
...
* {blank}
+
  ic0.canister_self_len : () -> (i32)
  ic0.canister_self_copy: (dst : i32, offset : i32, length : i32) -> ()+
+
These functions allow the canister to query its own canister id (as a blob). As usual, `_len` returns the length in bytes and `_copy` can be used to copy bytes from the blob into the Wasm heap.
...
+
This performs a function call to the canister specified by `callee_src/_len`, calling the method specified by `name_src/_len`, sending the data specified by `name_src/len`.
+
...
+
This system call traps if any of the `*_src+*_len` exceed the size of the WebAssembly memory.
...
* {blank}
+
  ic0.stable_write(offset : i32, src : i32, len : i32) -> ()
+
...
* {blank}
+
  ic0.stable_read(dst : i32, offset : i32, len : i32) > ()
+
...
* `+ic0.debug_print : (src : i32, len : i32) -> ()+`
+
...
2. The use of references, instead of binary blobs, to address entities (users, canisters), e.g. in `ic0.msg_caller` or in `ic0.call_simple`. Additional functions will be provided to convert between the transparent binary representation of entity ids and references.
...
copy_to_canister<es>(dst : i32, offset : i32, length : i32, data : blob) =
  if offset+length > |data| then Trap
  if dst+length > |es.wasm_state.store.mem| then Trap
  es.wasm_state.store.mem[dst..dst+length] := data[offset..offset+length]

copy_from_canister<es>(src : i32, length : i32) blob =
  if src+length > |es.wasm_state.store.mem| then Trap
  return es.wasm_state.store.mem[src..src+length]
...
ic0.msg_arg_data_copy<es>(dst:i32, offset:i32, length:i32) =
  if es.params.data = NoData then Trap
  copy_to_canister<es>(dst, offset, length, es.param.arg)

ic0.msg_caller_size() : i32 =
  if es.params.caller = NoCaller then Trap
  return |es.params.caller|

ic0.msg_caller_copy(dst:i32, offset:i32, length:i32) : i32 =
  if es.params.caller = NoCaller then Trap
  copy_to_canister<es>(dst, offset, length, es.params.caller)

ic0.msg_reject_code<es>() : i32 =
  es.params.reject_code

ic0.msg_reject_msg_len<es>() : i32 =
  if es.reject_code = 0 then Trap
  return |es.params.reject_msg|

ic0.msg_reject_msg_copy<es>(dst:i32, offset:i32, length:i32) : i32 =
  if es.reject_code = 0 then Trap
  copy_to_canister<es>(dst, offset, length, es.params.reject_msg)

ic0.msg_reply_data_append<es>(src : i32, length : i32) =
  if es.response ≠ NoResponse then Trap
  es.reply_params.arg := es.reply_params.arg · copy_from_canister<es>(src, length)

ic0.msg_reply<es>() =
  if es.response ≠ NoResponse then Trap
  es.response := Reply (es.reply_params.arg)

ic0.msg_reject<es>(src : i32, length : i32) =
  if es.response ≠ NoResponse then Trap
  es.response := Reject (CANISTER_REJECT, copy_from_canister<es>(src, length))

ic0.canister_self_len<es>() : i32 =
  return |es.wasm_state.self_id|

ic0.canister_self_copy(dst:i32, offset:i32, length:i32)  =
  copy_to_canister<es>(dst, offset, length, es.wasm_state.self_id)
...
ic0.call_simple<es>(
  ...
  callee := copy_from_canister<es>(callee_src, callee_len);
  method_name := copy_from_canister<es>(name_src, name_len);
  arg := copy_from_canister<es>(data_src, data_len);
...
ic0.stable_write<es>(offset : i32, src : i32, len : i32) -> ()
...
ic0.stable_read<es>(dst : i32, offset : i32, len : i32) > ()
  if offset+len > |es.wasm_state.stable_mem| then Trap
...
ic0.debug_print<es>(src : i32, len : i32) =
  return

ic0.trap<es>(src : i32, len : i32) =
  Trap
....
