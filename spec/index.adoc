...
provided address in the specified Bitcoin network based on the current view of the Bitcoin blockchain available to the Bitcoin component of the replica.
...
If the call fails, e.g., because the address is malformed, a `get_utxos_error` variant is returned,
...
There is an upper bound of 144 on the minimum number of confirmations because only the 144 most recent
blocks are cached and it is not possible to return the correct result based on the UTXO set.
If a larger minimum number of confirmations is specified, a `MinConfirmationsTooLarge` error is returned.
...
If the call fails, e.g., because the address is malformed, a `get_balance_error` variant is returned,
indicating the reason for the failed call.
...
If at least one of these checks fails, a `send_transaction_error` is returned,
indicating the reason for the failed call.

If the transaction passes these tests, the transaction is forwarded to the
specified Bitcoin network.

The Bitcoin component of the replica periodically forwards the transaction
...
[#ic-bitcoin_get_current_fees]
=== IC method `bitcoin_get_current_fees`
...
This function returns the fee percentiles, measured in Satoshi/byte, over the last 10,000 transactions, i.e.,
over the transactions in the last approximately 4-10 blocks.

The single function parameter of type u64 is the `step_size`, which is a positive integer between 1 and 100 (inclusive).
For example, a step size of 10 results in a response containing the 10th, 20th, ..., 100th percentile.
If the step size is 25, the response will contain the 25th, 50th, 75th, and 100th percentile.
