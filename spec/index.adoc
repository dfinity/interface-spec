...
[#system-api-canister-upgrades-and-dropping-stable-memory]
==== Canister upgrades and dropping stable memory

There is a special type of canister upgrade that drops stable memory at the end of the upgrade process. The purpose of this upgrade is to be used by canisters that use their stable memory as a scratchpad during upgrades. In such situations, the stable memory is not needed after the upgrade is finished and canisters can drop it, thus saving on resources and cycles cost.

NOTE: Dropping stable memory at the end of the upgrade can lead to data loss. Use it only in cases where you're certain that no significant data resides in your stable memory after an upgrade.

The IC executes this mode as follows:

 1. Invokes `canister_pre_upgrade` (if present) on the old instance, to give the canister a chance to clean up (e.g. move data to <<system-api-stable-memory,stable memory>>).
 2. Instantiates the new module, including the execution of `(start)`, with a fresh WebAssembly state.
 3. Invokes `canister_post_upgrade` (if present) on the new instance, passing the `arg` provided in the `install_code` call (<<ic-install_code>>).
 4. Discards any changes to stable memory, i.e. replace the existing stable memory of the canister with an empty one.

 Any other WebAssembly state is discarded.
...
* If `mode = upgrade_and_drop_stable_memory`, this will perform an upgrade of a non-empty canister with additionally dropping its stable memory as described in <<system-api-canister-upgrades-and-dropping-stable-memory>>, passing `arg` to the `canister_post_upgrade` method of the new instance.
...
==== IC Management Canister: Code upgrade and drop stable memory

Only the controllers of the given canister can upgrade a canister and drop its stable memory. This changes the code of an _existing_ canister and additionally drops its stable memory. This involves invoking the `canister_pre_upgrade` method on the old and `canister_post_upgrade` method on the new canister, which must succeed and must not invoke other methods.
...
....
    ...
    Public_custom_sections = parse_public_custom_sections(A.wasm_module)
    Private_custom_sections = parse_private_custom_sections(A.wasm_module)
    A.mode = upgrade_and_drop_stable_memory
    M.caller ∈ S.controllers[A.canister_id]
    S.canisters[A.canister_id] = { wasm_state = Old_state; module = Old_module, …}
    Env = {
      time = S.time[A.canister_id];
