...
The system can drop the oldest canister change entries from the list to keep its length bounded (at least 1000 change entries are guaranteed to remain in the list), but the indices of the remaining change entries are not affected.
 ...
 * `/canisters/<canister_id>/history`. Can be requested by anyone if `<canister_id>` matches `<effective_canister_id>`.
 * `/canisters/<canister_id>/total_num_changes`. Can be requested by anyone if `<canister_id>` matches `<effective_canister_id>`.
...
MethodCall = {
  ...
  callback: Callback;
  canister_history_index : CanisterHistoryIndex;
}
...
canister_change_origin(principal, FromCanister { canister_history_index = N, … }) = FromCanister {
    canister_id = principal
    canister_history_index = N
  }
canister_change_origin(principal, FromHeartbeat { canister_history_index = N, … }) = FromCanister {
    canister_id = principal
    canister_history_index = N
  }
....
...
S with
    ...
    call_contexts[Ctxt_id] = {
      ...
      origin = FromHeartbeat {
          canister_history_index = S.canister_history[C].total_num_changes;
        };
    ...
    messages =
      ...
      [ CallMessage {
          origin = FromCanister {
            ...
            canister_history_index = S.canister_history[M.receiver].total_num_changes;
          };
          caller = M.receiver;
          callee = call.callee;
...
may_read_path(S, _, ["canister", cid, "total_num_changes"]) = cid == ECID
...
state_tree(S) = {
  ...
  "canister":
    { canister_id :
        ...
        { "history" : { LEB128(i + S.canister_history[canister_id].total_num_changes - |S.canister_history[canister_id].change_entries|) : CBOR(S.canister_history[canister_id].change_entries[i]) | 0 ≤ i < |S.canister_history[canister_id].change_entries| } } ∪
