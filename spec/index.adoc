...
1. For each field that is present in the request (i.e. omitted optional fields are indeed omitted), including `request_type`, hash the fields name (in ascii-encoding, without terminal `\x00`) and the value (with the encoding specified below).
...
* No floating point globals are used in the module. (This may be allowed in the future.)
