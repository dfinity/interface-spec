...
- The canister address of the Bitcoin canister that uses the real Bitcoin blockchain is `TBD`.
- The canister address of the Bitcoin canister that uses the Bitcoin testnet is `TBD`.

It is *not* possible to interact with either Bitcoin canister via external requests (i.e., ingress messages).
...
least the provided number of confirmations, i.e., transactions with fewer than this number of
confirmations are not considered.
...
Since a response will contain at most `TBD` `Utxo` records, this parameter is useful for addresses
with many UTXOs.
There is no guarantee that the set of UTXOs will remain unchanged between function calls with different
...
all UTXOs returned by `get_utxos` for the same two parameters and returns the sum of 
UTXO values.
Since the function is executed on the replica, there is no pagination limit, i.e., `get_balance`
will iterate over all relevant UTXOs.
...
than it costs to spend the Bitcoin in the output, making the output worthless.
...
Bitcoin adapter, which offers it to all connected Bitcoin nodes.

The Bitcoin canister periodically forwards the transaction to the Bitcoin adapter
