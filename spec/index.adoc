...
Each request can specify the maximal expected size for the response body from the remote HTTP server. The upper limit on the size of a response body defaults to `2MiB` if no maximal size value is specified.
An error will be returned when the response body is larger than the maximal size.
...
- `max_response_bytes` - optional, specifies the maximal size of the response *body* in bytes. Any value less than or equal to `2MiB` is accepted. The call will be charged based on this parameter. If not provided, the maximum of `2MiB` will be used.
...
The size of the response (from the remote server if no `transform` function is specified or after `transform`) in Candid encoding (including `status`, `headers`, and `body`) must not exceed 2045952 (`2MiB - 50KiB`).
