...
* Needs updating: The “The DFINITY Protocol Overview” has empty canisters, and installation is a two-step process of installing an empty canister, and upgrading from that. This needs to be put in here.
...
* Minor post-merge todo: Use `args` instead of `payload` in the HTTP interface
 ...
 * `payload` (optional, blob): Initialization parameters
Response format:: JSON record with these fields
 * `request-nonce`: A unique number identifying the request

Uploading a new canister will install this canister, and invoke its `start` system method as explained in Section “<<system-api-start>>“, passing the `payload`, if given, as the method argument.

If the canister does not have a `start` system method, then it is an error to pass a `payload`. If no `payload` is given but a `start` system method is present, a payload of length 0 is assumed.
...
We specify the behaviour of the system using pseudo-code.
...
....
    ...
    S.to_install = { request = Req_id; canister = C; payload = P } · Other_to_install
    C.init P = Return { new_state = New_state; new_calls = (); response = NoResponse }
....
State after::
....
S with
