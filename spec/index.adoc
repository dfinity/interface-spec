...
NOTE: Once the IC frees the resources of a canister, its id, _cycles_ balance, _controllers_, canister _version_, and total number of canister change entries are preserved on the IC for a minimum of 10 years. What happens to the canister after this period is currently unspecified.
...
[#ic-canister-history]
=== IC method `canister_history`

Provides the history of the canister. Cycles to pay for the call must be explicitly transferred with the call, i.e., they are not deducted from the caller’s balance implicitly (e.g., as for inter-canister calls). Anyone can call this method.

The canister history consists of a list of canister change entries. Every canister change entry consists of the system timestamp at which the change was performed, the canister version after performing the change, the change's origin (a user or a canister), and its kind (canister creation, code uninstallation, code deployment, or controllers change). The change origin includes the principal (called _originator_ in the following) that initiated the change and, if the originator is a canister, the originator's canister version when the originator initiated the change. Code deployments are described by their mode (code install, code reinstall, code upgrade) and the SHA-256 hash of the newly deployed canister module. Controllers changes are described by the full new set of the canister controllers after the change.

The system can drop the oldest canister change entries from the list to keep its length bounded (at least `1000` change entries are guaranteed to remain in the list). The system also drops all canister change entries if the canister runs out of cycles.

The following parameters should be supplied for the call:

* `canister_id`: the canister ID of the canister whose history is requested.
* `num_requested_change_entries`: optional, specifies the number of requested canister change entries. The call will be charged based on this parameter. If not provided, the maximum of `1000` will be used.

The returned response contains the following fields:

* `total_num_change_entries`: the total number of canister change entries that have been ever recorded in the history. This value does not change if the system drops the oldest canister change entries from the list of change entries.
* `change_entries`: the list containing the most recent canister change entries. If `num_requested_change_entries` is provided, then this list contains that number of change entries or all change entries available in the system if more change entries are requested than available in the system.
...
....
canister_change_origin(principal, _, FromUser { … }) = FromUser {
    user_id = principal
  }
canister_change_origin(principal, sender_canister_version, FromCanister { … }) = FromCanister {
    canister_id = principal
    canister_version = sender_canister_version
  }
canister_change_origin(principal, sender_canister_version, FromSystemTask) = FromCanister {
    canister_id = principal
    canister_version = sender_canister_version
  }
....
...
The function `validate_sender_canister_version` checks that the `sender_canister_version` matches the actual canister version of the sender in all calls to the methods of the management canister that take `sender_canister_version`:
....
...
S with
    ...
    canister_history[CanisterId] = {
      total_num_change_entries = 1
      change_entries = {
        timestamp = CurrentTime
        canister_version = 0
        origin = canister_change_origin(M.caller, A.sender_canister_version, M.origin)
...
Once a canister runs out of cycles, its code is uninstalled (cf. <<rule-uninstall>>), the canister history change entries dropped, and the allocations are set to zero (NB: allocations are currently not modeled in the formal model):
