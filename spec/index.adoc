...
* If it exports a function called `dfn_init`, the function must have type `+(api_nonce : anyref) -> ()+`.
* If it exports any functions called `dfn_update <name>` or `dfn_query <name>` for some `name`, the functions must have type `+(api_nonce : anyref) -> ()+`.
