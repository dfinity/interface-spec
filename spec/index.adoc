...
The system can drop the oldest canister change entries from the list to keep its length bounded (at least `1000` change entries are guaranteed to remain in the list). The system also drop all canister change entries if the canister runs out of cycles.
...
The function `validate_sender_canister_version` checks that the `sender_canister_version` matches the actual canister version of the sender in all calls to the management canister:
....
validate_sender_canister_version(new_calls, canister_version_from_system) =
  ∀ call ∈ new_calls. (call.callee = ic_principal and call.arg = candid(A) and A.sender_canister_version = n) => n = canister_version_from_system
....
