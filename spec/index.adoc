...
If the canister does not have a `dfn_init` system method, then it is an error to pass a `arg`. If no `arg` is given but a `dfn_init` system method is present, an argument of length 0 is assumed.
...
Canister installation upload the initial canister module and turns an empty canister into a running canister. This involves invoking the `dfn_init` system method (see <<system-api-init>>), which must succeed and must not invoke other methods.
...
+
If the WebAssembly module does not export a function called under the name `dfn_init`, it is
+
....
