...
* 4xx HTTP status when the is error caused by the client.
* 5xx HTTP status when the server is aware that it has encountered an error or is otherwise incapable of performing the request. 
