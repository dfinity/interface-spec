...
* The fields `sender_pubkey` and `sender_sig` must be omitted if the sender the anonymous principal.
  The fields `sender_pubkey` and `sender_sig` must be set if the sender not the anonymous principal.
...
When querying the status of a request (see <<api-request-status>>) in the state tree, the user identifies the request using a _request id_, which is an object hash of the request's `content`, calculated using the following steps. The hash operation is always SHA-256.
...
verify_envelope({ content = C }, U, T)
  = U = anonymous_id
verify_envelope({ content = C, sender_pubkey = PK, sender_sig = Sig }, U, T)
...
....
  ...
  verify_envelope(E, R.sender, S.system_time)
