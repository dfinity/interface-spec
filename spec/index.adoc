...
NOTE: Once the IC frees the resources of a canister, its id, _cycles_ balance, _controllers_, and _history_ are preserved on the IC for a minimum of 10 years. What happens to the canister after this period is currently unspecified.
...
Users have the ability to learn about the hash of the canister's module, its current controllers, history, and metadata in a certified way.
...
* `/canister/<canister_id>/history/<change_index>` (blob):
+
The canister history consists of a list of canister change entries whose individual entries are accessible via this path. The `<change_index>` is encoded using the shortest form https://en.wikipedia.org/wiki/LEB128#Unsigned_LEB128[Unsigned LEB128] encoding. The value consists of a CBOR value encoding the canister history change entry according to the following CDDL (the https://tools.ietf.org/html/rfc8610[Concise Data Definition Language (CDDL)] is a data description language for CBOR). You can also link:{attachmentsdir}/history.cddl[download the file].

[source,bash]
----
include::{example}history.cddl[]
----

The system can drop the oldest canister change entries from the list to keep its length bounded (at least 1000 change entries are guaranteed to remain in the list), but the indices of the remaining change entries are not affected. The system state tree might not contain paths for all canister history change entries, but at least 20 most recent change entries (or all change entries if the canister history contains less than 20 change entries) are guaranteed to be present in the system state tree.

Every canister change entry consists of the system timestamp at which the change was performed, its origin (a user, a canister, or system), and its kind (canister creation, code change, or controllers change). The change origin can be from system if a canister is uninstalled because it ran out of cycles. Otherwise, the change origin includes the principal (called _originator_ in the following) that initiated the change and, if the originator is a canister, the index into the originator's canister history when the originator initiated the change (represented as the total number of the originator's changes at that time). Code changes are described by their kind (code uninstall, code install, code reinstall, code upgrade) and, if applicable, the SHA-256 hash of the newly deployed canister module. Controllers changes are described by the full new set of the canister controllers after the change.

* `/canister/<canister_id>/total_num_changes` (natural):
+
The total number of canister change entries that have been ever recorded in the history, encoded using the shortest form https://en.wikipedia.org/wiki/LEB128#Unsigned_LEB128[Unsigned LEB128] encoding. This value does not change if the system drops the oldest canister change entries from the list of change entries. In particular, if the value at this path is positive, then the latest change entry is available in the history at `<change_index>` equal to the value at this path minus one.
 ...
 * `/canisters/<canister_id>/history`. Can be requested if `<canister_id>` matches `<effective_canister_id>`.
 * `/canisters/<canister_id>/total_num_changes`. Can be requested if `<canister_id>` matches `<effective_canister_id>`.
...
Invoking the functions of the Bitcoin API will cost cycles. The concrete costs for each function are yet to be determined.
...
CanisterChangeEntry = {
  ...
  change : CanisterChange;
}
CanisterHistory = {
  total_num_changes : Nat;
  change_entries : [CanisterChangeEntry];
}
S = {
...
To convert `CallOrigin` into `CanisterChangeOrigin`, we define the following conversion function:
....
canister_change_origin(principal, FromUser { … }) = FromUser {
    user_id = principal
  }
canister_change_origin(principal, FromCanister { canister_state_counter = N, … }) = FromCanister {
    canister_id = principal
    canister_state_counter = N
  }
canister_change_origin(principal, FromHeartbeat { canister_state_counter = N, … }) = FromCanister {
    canister_id = principal
    canister_state_counter = N
  }
....
...
S with
    ...
    call_contexts[Ctxt_id] = {
      ...
      origin = FromHeartbeat {
          canister_state_counter = S.canister_state_counter[C];
        };
...
S with
    ...
    canister_history[CanisterId] = {
      ...
      change_entries = {
        ...
        origin = canister_change_origin(M.caller, M.origin)
        change = CanisterCreation
      }
    }
    messages = Older_messages · Younger_messages ·
      ResponseMessage {
...
S with
    ...
    canister_history[CanisterId] = {
      total_num_changes = N + 1;
      change_entries = H · {
          timestamp = S.time[CanisterId];
          canister_state_counter = S.canister_state_counter[CanisterId]
          origin = FromSystem;
          change = CodeUninstall;
        };
    }
...
The list of canister change entries can be trimmed, but the total number of recorded canister changes cannot be altered. At least 1000 change entries are guaranteed to remain in the list of change entries.

Conditions::
....
  S.canister_history[CanisterId] = {
      total_num_changes = N;
      change_entries = Older_change_entries · Newer_change_entries;
    }
  |Newer_change_entries| ≥ 1000
....
State after::
....
S with
    canister_history[CanisterId] = {
      total_num_changes = N;
      change_entries = Newer_change_entries;
    }
....
...
may_read_path(S, _, ["canister", cid, "history"] · _) = cid == ECID
may_read_path(S, _, ["canister", cid, "total_num_changes"] · _) = cid == ECID
...
state_tree(S) = {
  ...
  "canister":
    { canister_id :
        ...
        { "history" : { LEB128(i) : CBOR(S.canister_history[canister_id].change_entries[i]) | max(0, S.canister_history[canister_id].total_num_changes - 20) ≤ i < S.canister_history[canister_id].total_num_changes } } ∪
        { "total_num_changes" : LEB128(S.canister_history[canister_id].total_num_changes) } ∪
