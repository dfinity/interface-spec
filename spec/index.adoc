...
* The canister table may contain functions of type `+(env : i32) -> ()+` which may be used as callbacks for inter-canister calls made with the `ic0.call_new` function.
* The canister may export functions named `canister_callback <name>` and type `+(env : i64) -> ()+` which are used as callbacks for inter-canister calls made with the `ic0.call_new_name_callbacks` function.
...
Callbacks for inter-canister calls made with the `ic0.call_new` function are addressed by their table index (as a proxy for a Wasm `funcref`).
Callbacks for inter-canister calls made with the `ic0.call_new_name_callbacks` function are addressed by a _callback name_, and delivered to an exported function named `canister_callback <name>` with type `+(env : i64) -> () +`.
...
When handling an update call (or a callback), a canister can do further calls to another canister. Calls are assembled in a builder-like fashion, starting with `ic0.call_new` or `ic0.call_new_name_callbacks`, adding more attributes using the `ic0.call_*` functions, and eventually performing the call with `ic0.call_perform`.
...
This traps if the blob indicated by `name_src/_size` is not a valid utf8-encoded https://webassembly.github.io/spec/core/syntax/values.html#syntax-name[name].
...
This method begins assembling a call like `ic0.call_new` but with registering the callbacks by their function names and with using a callback function of type `+(env : i64) -> ()+`.

This traps if the blob indicated by `name_src/_size`, `reply_fun/_size` or `reject_fun/_size` is not a valid utf8-encoded https://webassembly.github.io/spec/core/syntax/values.html#syntax-name[name].

When the response comes back, then

 * If the response is a reply, the function `canister_callback <name>`, where `name` is the blob indicated in `reply_fun_src/size`, is executed with the corresponding `reply_env` value.
 * If the response is a reject, the function `canister_callback <name>`, where `name` is the blob indicated by `reject_fun_src/size`, is executed with the corresponding `reject_env` value.


NOTE: Each new call can be made with either `ic0.call_new` or `ic0.call_new_name_callbacks`. 

Subsequent calls to the following functions set further attributes of that call, until the call is concluded (with `ic0.call_perform`) or discarded (by returning without calling `ic0.call_perform` or by starting a new call with `ic0.call_new` or `ic0.call_new_name_callbacks`.)
--
...
There must be at most one call to `ic0.call_on_cleanup` between `ic0.call_new` or `ic0.call_new_name_callbacks` and `ic0.call_perform`.
--

* {blank}
+
--
  ic0.call_on_cleanup_name : (fun_src : i32, fun_size : i32, env : i64) -> ()

This method is an alternative to `ic0.call_on_cleanup` but with registering the cleanup callback by its name and with the type `+(env : i64) -> ()+`.

This traps if the blob indicated by `fun_src/_size` is not a valid utf8-encoded name.

If and only if the `reply` or the `reject` callback was executed and trapped (for any reason), the function `canister_callback <name>`, where `name` is the blob indicated in `fun_src/size`, is executed with the corresponding `env` value.

During the execution of the `cleanup` function, only a subset of the System API is available (namely `ic0.debug_print`, `ic0.trap` and the `ic0.stable_*` functions). The cleanup function is expected to run swiftly (within a fixed, yet to be specified cycle limit) and serves to free resources associated with the callback.

If this traps (e.g. runs out of cycles), the state changes from the `cleanup` function are discarded, as usual, and no further actions are taken related to that call. Canisters likely want to avoid this from happening.

There must be at most one call to `ic0.call_on_cleanup_name` between `ic0.call_new` or `ic0.call_new_name_callbacks` and `ic0.call_perform`.

NOTE: One of either `ic0.call_on_cleanup` or `ic0.call_on_cleanup_name` can be used to specify a cleanup callback but not both.
--
...
Callback = {
  on_reply : Closure | ClosureNameCallback;
  on_reject : Closure | ClosureNameCallback;
  on_cleanup : Closure | ClosureNameCallback | NoClosure;
}
....
...
For callbacks registered by name, the following function applies, where `exports` is the set of exported functions.
invoke_name_callback(es, callback) =
...
callbacks = λ(callbacks, response, refunded_cycles, sysenv, available) → λ wasm_state →
  ...
  let (callback, params, context) = match response with
    Reply data ->
      (callbacks.on_reply, { params0 with data }, Ry)
    Reject (reject_code, reject_message)->
      (callbacks.on_reject, { params0 with reject_code; reject_message }, Rt)
  let es = ref {empty_execution_state with
    wasm_state = wasm_state;
    params = params;
    balance = sysenv.balance;
  ...
  try
    if callback.is_by_table_index then { 
      if callback.fun > |es.wasm_state.store.table| then Trap
      let func = es.wasm_state.store.table[fun]
      if typeof(func) ≠ func (i32) -> () then Trap
      func<es>(env)
    } else if callback.is_by_name {
      invoke_name_callback(es, callback)
    }
    Return {
      new_state = es.wasm_state;
  ...
  with Trap
    ...
    if callback.is_by_table_index then {
      ...
      try invoke_name_callback(es', callbacks.on_cleanup) with Trap then Trap {cycles_used = es.cycles_used + es'.cycles_used;}
    }
    Return {
      new_state = es'.wasm_state;
      new_calls = [];
...
ic0.call_new<es>(
  ...
  if not valid_utf8(method_name) then Trap {cycles_used = es.cycles_used;}
...
ic0.call_new_name_callbacks<es>(
    callee_src      : i32,
    callee_size     : i32,
    name_src        : i32,
    name_size       : i32,
  ...
  if not valid_utf8(reply_fun) then Trap {cycles_used = es.cycles_used;}
  reject_fun := copy_from_canister<es>(reject_fun_src, reject_fun_size);
  if not valid_utf8(reject_fun) then Trap {cycles_used = es.cycles_used;}
  ...
  es.pending_call = MethodCall {
    ...
    transferred_cycles = 0;
    callback = Callback {
      on_reply = ClosureNameCallback { fun = reply_fun; env = reply_env }
      on_reject = ClosureNameCallback { fun = reject_fun; env = reject_env }
      on_cleanup = NoClosure
    };
  }
...
ic0.call_on_cleanup_name<es> (fun_src : i32, fun_size : i32, env : i64) =
  if es.pending_call = NoPendingCall then Trap {cycles_used = es.cycles_used;}
  if es.pending_call.callback.on_cleanup ≠ NoClosure then Trap {cycles_used = es.cycles_used;}
  fun := copy_from_canister<es>(fun_src, fun_size);
  if not valid_utf8(fun) then Trap {cycles_used = es.cycles_used;}
  es.pending_call.callback.on_cleanup := ClosureNameCallback { fun = fun; env = env }
