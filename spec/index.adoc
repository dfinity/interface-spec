...
Within the `canister_pay_ingress` function, the canister may inspect the identity of the caller (using `ic0.msg_caller_*`) and the name of the invoked method (using `ic0.msg_method_*`). If access should be granted, the canister calls the `ic0.pay_for_ingress` system function.
...
verify_envelope(E, U) =
  ...
  || ∃ PK, SIG.
       ...
       && verify_signature E.sender_pubkey E.sender_sig (request_id_of(E.content)) = true
....
