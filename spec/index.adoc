...
- The canister address of the Bitcoin canister that uses the Bitcoin testnet is `jf4vc-xabaa-aaaaa-aaaaa-eai`.
...
Given a https://en.bitcoin.it/wiki/Base58Check_encoding[base58-encoded] address as part of the
`GetBalanceRequest` , the function returns the current balance of this address in `Satoshi` (10^8 Satoshi = 1 Bitcoin).
...
all UTXOs, i.e., the same balance is returned as when calling `get_utxos`
