...
For example, the condition `S.messages = Younger_messages · M · Older_messages` says that `M` is some message in field `messages` of the record `S`, and that `Younger_messages` and `Older_messages` are the other messages in the system. If the “state after” specifies `S with messages = Younger_messages · Older_messages`, then the message `M` is removed from the state.
