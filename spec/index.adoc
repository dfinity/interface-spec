...
* Missing: Reads may specify a specific (future) state
...
NOTE: I expect that the message id will be calculated from on the message data, so there is no explicit `message_id` field below, but this is contested, and adding a `message_id` field will not fundamentally affect this design.
...
Both problems can be solved if read requests can specify a desired state that they should be queried again, either at-least-this (to solve the first problem) or an exactly-this-future-state (to solve the second). This requires some way of identifying states (abstract state counters, timestamps, block heights).

NOTE: Even without this feature, applications can solve these problems: For the first problem, the query result could be such that the client can tell if the query has been received or not. For the second problem, if the reply is monotonous in some sense the client can get assurance in their intersection (e.g. if the query returns a list of events that grows over time, then even if different nodes return different lists, the client can get assurance in those events returned by many nodes).
...
For all endpoints, the client performs a POST HTTP request with `Content-type: application/json`. The body is an JSON object containing the message resp. read request object.

The `/api/v1/submit` endpoint returns, upon successful submission, a (code 200) HTTP response without a body; the user uses a separate `message-status` read (see <<api-status>>) to determine the response. In contrast, a successful read via `/api/v1/read` returns a response (a JSON object) as the body of the (code 200) HTTP response. In both cases case, the usual HTTP errors (e.g. 500) may occur.
...
This will instantiate the canister module and invoke its `start` system method, as explained in Section “<<system-api-start>>“, passing the `arg`, if given, as the method argument.
...
This is atomic: If the response to this message is a `reject`, then this message had no effect. It is an error to invoke `install_code` on a canister again after a previous code installation succeeded; canister upgrades are handled separately. (This may be relaxed later, in particular if we switch to a non-serializing approach to persistent storage.)
...
If the canister exports a system method called `start`, then this is the first exported WebAssembly function invoked by the system. If an argument was passed along with the actor initialization (see <<api-install-code>>), it is passed as the argument, otherwise the argument is the empty blob.
...
NOTE: These messages are a different thing from the messages that occur in the RPC API; the latter are called API Messages
...
Note that all gray states are _not_ represented in the system state, and are indistinguishable from “message does not exist”. In order to avoid replay-attacks, messages have a limited validity, and the last transition (forgetting the message)
...
A `ReadStatus` refers to a message by way of a _message id_, which is a hash of a message content:
....
MessageId = Blob
...
Canister registration api message have the effect of allocating a fresh canister id:
...
Canister installation upload the initial canister module and turns an empty canister into a running canister. This involves invoking the `start` system method (see <<system-api-start>>), which must succeed and must not invoke other methods.
...
A first step in processing a canister update calls is to create a `CallMessage` in the message queue.
...
Canister query calls are executed immediately. They are not provided randomness.
...
....
  ...
  F = C.module.exports[Q.method_name]
....
Read response::
* If `F(NoEntropy, Q.callee, Q.arg) = Trap` then a JSON record with
+
....
{status: failed; error: "Query execution trapped"}
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Ok res` and `res.new_calls ≠ ()` then a JSON record with
+
....
{status: failed; error: "Query execution tried to send further messages "}
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Ok res` and `res.response = NoResponse` then a JSON record with
+
....
{status: failed; error: "Query execution did not issue a result" }
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Ok res` and `res.response = Reject errcode` then a JSON record with
+
....
{status: failed; error: <toErrorMessage errcode> }
....
* Else if `F(NoEntropy, Q.callee, Q.arg) = Ok res` and `res.response = Reply R` then a JSON record with
+
....
