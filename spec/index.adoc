...
NOTE: Once the IC frees the resources of a canister, its id, _cycles_ balance, _controllers_, and _history_ (including canister _version_) are preserved on the IC for a minimum of 10 years. What happens to the canister after this period is currently unspecified.
...
S with
    ...
    canister_history[CanisterId] = {
      ...
      change_entries = H · {
          ...
          canister_version = S.canister_version[CanisterId] + 1
          origin = FromSystem;
          change = CodeUninstall;
        };
    }
