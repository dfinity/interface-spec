...
Conceptually, the system state is a tree with labeled children, and values in the leafs. Equivalently, the system state is a mapping from paths (sequences of labels) to values, where the domain is prefix-free.
...
This section describes specifies the publicly relevant paths in the tree.
...
+
If the status is `rejected`, then this path contains the reject code (seee <<reject-codes>>), else it is not present.

* `/request_status/<request_id>/reject_message` (text)
+
If the status is `rejected`, then this path contains a textual diagnostic mmessage, else it is not present.

NOTE: Immediately after submitting a request, the request may not show up yet even though the system is still working on accepting the request as pending.
...
All requests coming in via the HTTP interface need to be _authenticated_ using a cryptographic signature. To that end, the following fields are added to all request records (next to `request_type`):
...
Response fields::
None, the server returns no body and HTTP status 202.
 ...
 * `/request_status/<request_id>`. Can only be read if this read was signed by same sender as the original request.
...
* Natural numbers (`compute_allocation`, `memory_allocation`, `ingress_expiry`) are encoded using https://en.wikipedia.org/wiki/LEB128#Unsigned_LEB128[Unsigned LEB128] encoding.
 ...
 * a sequence (typically 0 or 1) of _delegations_ that link the public key to _root public key_.
...
A certificate is validated with regard to the root of trust by the following algorithm (which uses `check_delegations` defined in <<certification-delegation>>):
....
verify_cert(cert) =
  let root_hash = reconstruct(cert.tree)
  let key = check_delegations(cert.delegations) // see section Delegations below
...
....
where `H` is the SHA-256 hash function and
....
verify_bls_signature : PublicKey -> Signature -> Blob -> Bool
....
is the https://tools.ietf.org/html/draft-irtf-cfrg-bls-signature-02#section-4[BLS signature verification function], ciphersuite BLS_SIG_BLS12381G1_XMD:SHA-256_SSWU_RO_NUL_. See that document also for details on the encoding of BLS public keys and signatures. The DER encoding of the public key follows https://tools.ietf.org/html/rfc5480[RFC4580] with (at least temorarily) using OID 1.3.6.1.4.1.44668.5.3.1.2.1 for the algorithm and 1.3.6.1.4.1.44668.5.3.2.1 for the curve.
...
The exact form of delegations is still under discussion; as scaffolding we use an insecure unsigned delegation:
....
Delegation =
 InsecureDelegation {
   key : PublicKey;
...
Additional forms of delegations will be added that would contain a signature or certificate linking the `key` to the root key.

A chain of delegations is verified using the following algorithm, which also returns the delegated key:

....
check_delegations(InsecureDelegation d :: ds) : public_bls_key =
  return key
check_delegations([]) : public_bls_key =
...
CanisterModule = {
  ...
  update_methods : MethodName ↦ ((Arg, Env, AvailableFunds) -> UpdateFunc)
  query_methods : MethodName ↦ ((Arg, Env) -> QueryFunc)
  callbacks : (Callback, Response, Funds, Env, AvailableFunds) -> UpdateFunc
...
A `Path` may refer to a request by way of a _request id_, as specifeid in <<api-request-id>>:
....
Request = Blob
...
In particular, the function implements https://ed25519.cr.yp.to/index.html[*Ed25519*], https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.186-4.pdf[*ECDSA*] or *BLS* signatures, depending on the type of the public key, for the plain format, as well as ECDSA signatures for the WebAuthn format.
....
PublicKey = Blob
Signature = Blob
...
....
for some time stamp `T`, some BLS public key PublicKey, and using `()` to denote the empty map or bag.
...
....
    ...
    Env = { time = S.time[M.receiver]; balance = Balance_before }
...
....
    ...
    Env = { time = S.time[M.receiver]; balance = S.balances[M.receiver] }
...
....
  ...
  Env = { time = S.time[M.receiver]; canister_location = S.canister_location; balance = S.balances[Q.canister_id] }
....
Read response::
...
may_read_path(S, _, ["request_status", Rid] · _) =
  if ∃ R ∈ S.requests ∧ request_id_of(R) = Rid
...
....
where `state_tree` constructs the a labeled tree from the system state `S` (as per <<state-tree>>)
....
state_tree(S) = {
  "time": S.system_time;
