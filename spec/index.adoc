...
This method returns the threshold ECDSA public key for the given canister using the given derivation path. If the `canister_id` is unspecified, it will default to the canister id of the caller. The derivation path is a vector of variable length byte strings.
...
This method returns a new threshold ECDSA signature of the given `message_hash` that can be separately verified against the caller's ECDSA public key derived from the caller's canister id and the given derivation path.
