...
* {blank}
+
  ic.call_perform : ( callref : anyref ) -> ()
+
...
The `CanId` parameter of the `InitFunc` is merely passed through to the canister, via the `canister.self` system call.
...
Conditions::
....
    S.requests[M] = Received
    Code = SYS_FATAL or Code = SYS_TRANSIENT
....
State after::
....
S with
    requests[M] = Rejected (Code, Msg)
....
...
....
Read response::
* If `F(Q.arg) = Trap` then
+
....
{status: failed; error: "Query execution trapped"}
....
* Else if `F(Q.arg) = Return (Reject (code, msg))` then
+
....
{status: rejected; reject_code: <code>: reject_message: <msg>}
....
* Else if `F(Q.arg) = Return (Reply R)` then
+
....
...
+
....
update_methods[method] = λ blob → λ wasm_state →
  let es = ref {empty_execution_state with
...
+
....
query_methods[method] = λ blob → λ wasm_state →
  let es = ref {empty_execution_state with
      wasm_state = wasm_state;
      api_nonce = fresh_nonce();
      params = { arg = blog; reject_code = 0; reject_message ""}
...
ic.canister_self<es>(api_nonce : anyref) : anyref =
  if api_nonce ≠ APIRef es.api_nonce then Trap
  return (CanisterRef es.wasm_state.self_id)
