...
[#ic-bitcoin-canisters]
== The IC bitcoin canisters

Similar to the <<ic-management-canister,management canister>>, the Bitcoin functionality is also exposed through a facade, which we refer to as the "Bitcoin canisters".
There are two Bitcoin canisters differing only in that one operates on the real Bitcoin blockchain whereas the other only works with the Bitcoin testnet.
The Bitcoin canisters can be accessed using their addresses:

- The canister address of the Bitcoin canister that uses the real Bitcoin blockchain is `u25tu-yqaaa-aaaaa-aaaaa-eai`.
- The canister address of the Bitcoin canister that uses the Bitcoin testnet is `u54va-viaaa-aaaaa-aaaaq-eai`.

Invoking functions on these canisters will cost cycles. The concrete costs for each function are yet to be determined.
...
The following interface description, in https://github.com/dfinity/candid/blob/master/spec/Candid.md[Candid syntax], describes the available functionality. You can also link:{attachmentsdir}/bitcoin.did[download the file].
----
include::{example}bitcoin.did[]
----
...
Given a https://en.bitcoin.it/wiki/Base58Check_encoding[base58-encoded] address as part of the
`GetUtxosRequest` , the function returns all unspent transaction outputs (UTXOs) associated with the
provided address based on the current view of the Bitcoin blockchain available to the Bitcoin canister.
...
If the call fails, e.g., because the address is malformed, a `GetUtxosError` variant is returned,
...
minimum number of confirmations or a pagination record.
...
In the second case, a pagination record must be provided, specifying a block hash that constitutes
the tip of the considered chain for this request and, optionally, an offset and a limit.
A `GetUtxosRequest` without the optional `filter` results in a request that
considers the full blockchain, which is equivalent to setting
`MinConfirmations` to 0.

The recommended workflow is to issue a request with the desired number of confirmations.
If the `total_count` in the response is greater then the number of returned UTXOs, the
tip (block) hash in the response can then be used for a subsequent pagination request to gather
additional UTXOs, setting the offset to the number of received UTXOs.

[#ic-bitcoin_get_balance]
=== IC method `get_balance`

Given a https://en.bitcoin.it/wiki/Base58Check_encoding[base58-encoded] address as part of the
`GetBalanceRequest` , the function returns the current balance of this address in `Satoshi` (10^8 Satoshi = 1 Bitcoin). The same address formats as for link:#ic-bitcoin_get_utxos[`get_utxos`] are supported.

If the call fails, e.g., because the address is malformed, a `GetBalanceError` variant is returned,
indicating the reason for the failed call.
...
in the same manner as for the `get_utxos` call.

Given an address and the optional `min_confirmations` parameter, `get_balance` iterates over
all UTXOs, i.e., the same balance is returned as when calling link:#ic-bitcoin_get_utxos[`get_utxos`]
...
[#ic-send_transaction]
=== IC method `send_transaction`

Given a `blob` of a Bitcoin transaction, several checks are performed:
...
The Bitcoin canister periodically forwards the transaction
...
the Bitcoin canister drops the transaction.
