...
* `signatures`: Array of zero or more objects with fields described as follows.
...
* `controller` (`PrincipalId`): The new controller of the canister.
Reply fields::
* None
...
....
verify_envelope(E, U) =
     is_anonymous_id U && E.signatures = []
  || ∃ PK, SIG.
       E.signatures = [{ sender_pubkey = PK, sender_sig = SIG }]
       && is_self_authenticating_id PK E.content.canister_id U
       && verify_signature PK SIG (request_id_of(E.content)) = true
....
