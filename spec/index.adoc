...
+
Maximum canister cycle balance. Any excess is discarded. Less than 2⁶⁴.
...
NOTE: This specification currently does not go into details about which actions cost how many cycles and/or when. In general, assume that the canister’s cycle balance can change arbitrarily between method executions, and during each system function API call, unless explicitly mentioned otherwise.
...
+
NOTE: Should a future version of this specification allow `MAX_CANISTER_BALANCE` >= 2⁶⁴, then this call will report 2⁶⁴-1 if the balance exceeds 2⁶⁴. A new, wider System API will then be provided for canisters that need to deal precisely with large canister balances.
