...
init = λ (self_id, arg) →
  let es = ref {empty_execution_state with
      ...
      params = { data = arg.data; caller = arg.caller; reject_code = 0; reject_message ""; time = arg.time}
    }
...
pre_upgrade = λ (old_state, caller, time) →
  let es = ref {empty_execution_state with
      ...
      params = { data = NoData; caller = caller; reject_code = 0; reject_message - ""; time = time}
    }
  try func<es>() with Trap then Trap
  if es.performed_calls ≠ [] then Trap
  if es.response ≠ NoResponse then Trap
