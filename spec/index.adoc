...
Internally, the calls are implemented using _messages_ from a _sender_ to a _receiver_, messages do not have a response.
...
1. A client submits a request via the `submit` RPC endpoint. No useful information is returned from the node (as it would not be trustworthy anyways).
2. For a certain amount of time, the system behaves as if it does not know about the request.
3. At some point, the system agrees to accept the request. From now on, the client can ask any node (of the right subnet) about the status of the request. The system is now processing the request.
4. At some later point, the request is acted upon. For some requests this is atomically, for others this involves multiple internal steps.
...
For all endpoints, the client performs a POST HTTP request with `Content-type: application/cbor`. The body is an CBOR value containing the request object.
...
The status `accepted` is used for requests that have successfully entered the system, known to all nodes, and that are guaranteed to be acted upon eventually.

WARNING: Immediately after submitting a request, this may fail (e.g. return with `unknown`) even though the system is still working on accepting the request.
...
An API request or inter-canister call that was accepted into the system will eventually result in either a _reply_ (indicating success, and carrying data) or a _reject_ (indicating an error of some sorts). A reject contains a _rejection code_ that classifies the error and a (hopefully) helpful error message string.
...
* If it exports any functions called `dfn_update <name>` or `dfn_query <name>` for some `name`, it must have type `+(api_nonce : anyref) -> ()+`.
...
To define a public method of name `name`, a WebAssembly module exports a function with name `dfn_update <name>` or `dfn_query <name>` and type `+(api_nonce : anyref) -> ()+`. We call this the _method entry point_. The name of the exported function distinguishes update and query methods.
...
* The partial map `update_methods` of the `CanisterModule` is defined for all method names `method` for which the WebAssembly program exports a function `f` named `dfn_update <method>`, and has value
+
....
...
* The partial map `query_methods` of the `CanisterModule` is defined for all method names `method` for which the WebAssembly program exports a function `f` named `dfn_query <method>`, and has value
+
....
