...
The `queue` field is used to describe the message ordering behavior. Its concrete value is only used to determine when the relative order of two messages must be preserved, and is otherwise not interpreted.
...
The function `reverse_queue` is undefined for origins of the form `FromSystemTask` since responding to messages generated by the system is not allowed.
...
messages = Older_messages · Younger_messages  ·
  ResponseMessage {
      ...
      queue = reverse_queue(S, CM.callee, CM.origin);
  }
  ...
  S with
    ...
    messages =
      ...
      [ ResponseMessage {
          ...
          queue = reverse_queue(S, M.receiver, S.call_contexts[M.call_context].origin);
        }
...
S with
    ...
    messages =
      ...
      ResponseMessage {
        ...
        queue = reverse_queue(S, S.call_contexts[Ctxt_id].canister_id, S.call_contexts[Ctxt_id].origin)
      }
...
S with
    ...
    messages = Older_messages · Younger_messages ·
      ResponseMessage {
        ...
        refunded_cycles = 0
        queue = reverse_queue(S, M.callee, M.origin)
      }
...
S with
    ...
    messages = Older_messages · Younger_messages ·
      ...
      [ ResponseMessage {
          ...
          queue = reverse_queue(S, Ctxt.canister, Ctxt.origin)
        }
...
S with
    ...
    messages = S.Messages ·
        [ ResponseMessage {
            ...
            response = Reply (candid())
            refunded_cycles = C
            queue = reverse_queue(S, ic_principal, O)
          }
        | (O, C) ∈ Origins
        ]
...
(RM.queue = Unordered) or (∀ msg ∈ Older_messages. msg.queue ≠ RM.queue)
