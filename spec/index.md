...
Canister methods that do not change the canister state can be executed more efficiently. A query call provides that ability: it is a fast, but less secure way to call a canister. Only methods that are explicitly marked as "query methods" by the canister can be called this way.
...
Canister query calls to `/api/v2/canister/<ECID>/query` and `/api/v2/canister/<ECID>/signed_query` can be executed directly. They can only be executed against canisters which have a status of `Running` and are also not frozen.
...
-   Else if `F(Q.Arg, Q.sender, Env) = Return {response = Reject (code, msg); …}` then
        ...
        {status: "rejected"; reject_code: <code>: reject_message: <msg>, error_code: <implementation-specific>, signatures: Sigs, certificate: Cert'}

-   Else if `F(Q.Arg, Q.sender, Env) = Return {response = Reply Res; …}` then
